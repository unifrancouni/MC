Public Class frmSteSolicitudProveedor
    ...
    Private Sub Seguridad()
        Try
            ...
            If Seg.HasPermission("AgregarRendicionCheque") = False Then
                Me.AgregarRendicionToolStripMenuItem.Enabled = False

            Else  'Habilita
                Me.AgregarRendicionToolStripMenuItem.Enabled = True
            End If


            If Seg.HasPermission("AprobarRendicionCheque") = False Then

                Me.AprobarRendicionToolStripMenuItem.Enabled = False
            Else  'Habilita
                Me.AprobarRendicionToolStripMenuItem.Enabled = True
            End If
    ...
    Private Sub FormatoSolicitudCheque()
        Try
            ...
            Me.grdSolicitud.Splits(0).DisplayColumns("nNumero").Visible = False
            Me.grdSolicitud.Splits(0).DisplayColumns("nSteSolicitudChequeProveedorID").Visible = False
            ...
            Me.grdSolicitud.Columns("nTipoDesembolso").Caption = "Transferencia"
            ...
            'Check Box:
            Me.grdSolicitud.Columns("nTipoDesembolso").ValueItems.Presentation = C1.Win.C1TrueDBGrid.PresentationEnum.CheckBox
            Me.grdSolicitud.Splits(0).DisplayColumns("nTipoDesembolso").Style.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
    ...
    Private Sub LlamaImprimirTE54()
        ...
        Try
            ...
            'frmVisor.CRVReportes.SelectionFormula = " {vwSteSolicitudDesembolsoProveedor.nSteSolicitudDesembolsoProveedorID} =" & XdsSolicitudCheque("Solicitud").ValueField("nSteSolicitudDesembolsoProveedorID")
    ...
    Private Sub LlamaImprimirTE56()
        ...
        Try
            ...
            frmVisor.SQLQuery = " SELECT * " & _
                                 " FROM  vwSteSolicitudDesembolsoProveedor " & " WHERE (CONVERT(Varchar(10), dFechaSolicitudDesembolso, 112) > CONVERT(Varchar(10), '" & Format(DateAdd(DateInterval.Day, -1, CDate(cdeFechaD.Text)), "yyyy-MM-dd") & "', 112)) AND (CONVERT(Varchar(10), dFechaSolicitudDesembolso, 112)  <= CONVERT(Varchar(10), '" & Format(CDate(cdeFechaH.Text), "yyyy-MM-dd") & "', 112)) " & _
                            " Order by nCodigo " 'Option (Force Order)"
    ...
    Private Sub LlamaAgregarRendicionCheque()
        Dim ObjfrmSteRendirCheque As New frmSteRendicionCheque
        Try
            Dim XcDatos As New BOSistema.Win.XComando
            Dim XdtSolicitudCheque As New BOSistema.Win.XDataSet.xDataTable
            Dim IDSolicitudCheque As Integer
            Dim Strsql As String = ""
            'No existen registros:
            If Me.grdSolicitud.RowCount = 0 Then
                MsgBox("No Existen registros grabados.", MsgBoxStyle.Information)
            ...
            If XdsSolicitudCheque("Solicitud").ValueField("sCodigoInterno") <> 3 Then
                MsgBox("Solicitud no se encuentra en estado Aprobado, no puede Rendir el cheque.", MsgBoxStyle.Information)
                Exit Sub
            End If

            Strsql = " select nSteSolicitudChequeProveedorID  from SteSolicitudChequeProveedor where nSteSolicitudDesembolsoProveedorID =" & XdsSolicitudCheque("Solicitud").ValueField("nSteSolicitudDesembolsoProveedorID")
            XdtSolicitudCheque.ExecuteSql(Strsql)
            If Not IsDBNull(XdtSolicitudCheque.Table.Rows(0).Item(0)) Then
                IDSolicitudCheque = Convert.ToInt32(XdtSolicitudCheque.Table.Rows(0).Item("nSteSolicitudChequeProveedorID"))
            End If


            '-- Ubicar cursor en un estado de ocupado
            Me.Cursor = System.Windows.Forms.Cursors.WaitCursor
            'ObjfrmSteRendirCheque.MdiParent = Me.MdiParent
            ObjfrmSteRendirCheque.ModoAccion = "ELA" ' StrAccion
            ObjfrmSteRendirCheque.IdDesembolso = XdsSolicitudCheque("Solicitud").ValueField("nSteSolicitudDesembolsoProveedorID")
            ObjfrmSteRendirCheque.IdSolicitudCheque = IDSolicitudCheque
            ObjfrmSteRendirCheque.WindowState = FormWindowState.Maximized
            ObjfrmSteRendirCheque.sColorFrm = "Naranja"
            '-- Para enviar el foco al formulario 
            '-- que se está llamando.
            ObjfrmSteRendirCheque.BringToFront()
            '-- Poner el cursor en un estado de desocupado
            Me.Cursor = System.Windows.Forms.Cursors.Default
            ObjfrmSteRendirCheque.ShowDialog()
        Catch ex As Exception
            Control_Error(ex)
        Finally
            ObjfrmSteRendirCheque = Nothing
        End Try
    End Sub

    Private Sub LlamaAutorizarRendicionCheque()
        Dim ObjfrmSteRendirCheque As New frmSteRendicionCheque
        Try
            Dim XcDatos As New BOSistema.Win.XComando
            Dim XdtSolicitudCheque As New BOSistema.Win.XDataSet.xDataTable
            ...
            If XdsSolicitudCheque("Solicitud").ValueField("sCodigoInterno") = 2 Then
                MsgBox("Solicitud se encuentra en estado Anulado, no puede Rendir el cheque.", MsgBoxStyle.Information)
                Exit Sub
            End If
            ...
            '-- Ubicar cursor en un estado de ocupado
            Me.Cursor = System.Windows.Forms.Cursors.WaitCursor
            ObjfrmSteRendirCheque.ModoAccion = "AUT" ' StrAccion
            ObjfrmSteRendirCheque.IdDesembolso = XdsSolicitudCheque("Solicitud").ValueField("nSteSolicitudDesembolsoProveedorID")
            ObjfrmSteRendirCheque.IdSolicitudCheque = IDSolicitudCheque
            ObjfrmSteRendirCheque.WindowState = FormWindowState.Maximized
    ...
    Private Sub AgregarRendicionToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AgregarRendicionToolStripMenuItem.Click
        Me.LlamaAgregarRendicionCheque()
    End Sub

    Private Sub AprobarRendicionToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AprobarRendicionToolStripMenuItem.Click
        Me.LlamaAutorizarRendicionCheque()
    End Sub
End Class