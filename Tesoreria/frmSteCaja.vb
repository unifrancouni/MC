...
Public Class frmSteCaja
    ...
    Private Sub tbCaja_ItemClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tbCaja.ItemClicked
        ...
        Select Case e.ClickedItem.Name
            ...
            Case "toolReabrir"
                LlamaReabrirCaja()
            Case "toolReabrirTodas"
                LlamaReabrirCajaTodas()
            Case "toolAyuda"
                LlamaAyuda()
        End Select
    End Sub
    ' --------------------------------------------------------------------------------------------------------
    ' Fecha:                04/09/2014
    ' Procedure Name:       LlamaReabrirCaja
    ' Descripción:          Este procedimiento permite inactivar una Caja y de inmediato crearla nuevamente.
    '---------------------------------------------------------------------------------------------------------
    Private Sub LlamaReabrirCaja()
        Try
            'Si la Caja ya encuentra Inactiva
            If XdtCaja.ValueField("nCerrada") = 1 Then
                MsgBox("La Caja ya está inactiva.", MsgBoxStyle.Information)
                Exit Sub
            End If

            ' Primero la caja será inactivada con el procedimiento correspondiente
            LlamaInactivarCaja("Reabrir")
    ...
    ' ----------------------------------------------------------------------------------------------------------------
    ' Fecha:                10/09/2014
    ' Procedure Name:       LlamaReabrirCajaTodas
    ' Descripción:          Este procedimiento permite inactivar TODAS las Cajas y de inmediato crearla nuevamente.
    '-----------------------------------------------------------------------------------------------------------------
    Private Sub LlamaReabrirCajaTodas()
        Dim XdtTmp As New BOSistema.Win.XDataSet.xDataTable

        Try

            If MsgBox("¿Esta seguro que desea reabrir todas las cajas?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then
                Exit Sub
            End If

            'Indicador de Actualización: 
            XdtTmp.ExecuteSql("SpSteCajasReabrirLote")

            If XdtTmp.Count > 0 Then
                MsgBox("No todas las cajas abiertas tienen sus arqueos cerrados. Por favor revise.", MsgBoxStyle.Information)
            Else
                MsgBox("Las cajas se reabrieron exitosamente.")
            End If
    ...
    Private Sub LlamaInactivarCaja(ByVal operacion As String)
        Dim ObjFrmSteEditCaja As New frmSteEditCaja
        ...
        Try
            ...
            Else
                ...
                res = MsgBox("¿Desea reabrir esta caja?", vbYesNo)
                Select Case res
                    Case vbYes
                        XdsCaja = New BOSistema.Win.XDataSet
                        StrSql = " Update SteCaja " & _
                                 " SET nCerrada = 1, dFechaCierre = getdate()," & _
                                 "     nUsuarioModificacionID = " & InfoSistema.IDCuenta & ", dFechaModificacion = getdate() " & _
                                 " WHERE nSteCajaID = " & XdtCaja.ValueField("nSteCajaID")
                        ...
                        ' Ahora vamos a crear un nuevo registro a partir del anteriormente inactivado
                        LlamaCrearRegistroCaja()
    ...
    Private Sub LlamaCrearRegistroCaja()
        ...
        Try
            ...
            ObjCajadt.Filter = "nSteCajaID = 0 " '& XdtCaja.ValueField("nSteCajaID")
            ObjCajadt.Retrieve()

            XdsCaja = New BOSistema.Win.XDataSet
            strSQL = " SELECT * FROM SteCaja WHERE nSteCajaID = " & XdtCaja.ValueField("nSteCajaID")
            If XdsCaja.ExistTable("Caja") Then
                XdsCaja("Caja").ExecuteSql(strSQL)
            Else
                XdsCaja.NewTable(strSQL, "Caja")
            ...
            ObjCajadr.nStbDelegacionProgramaID = XdsCaja("Caja").ValueField("nStbDelegacionProgramaID")

            'Fecha de Apertura: 
            ObjCajadr.dFechaApertura = FechaServer()

            'Tipo de Programa:
            ObjCajadr.nStbTipoProgramaID = XdsCaja("Caja").ValueField("nStbTipoProgramaID")

            'Plazo de Pagos:
            ObjCajadr.nStbTipoPlazoPagosID = XdsCaja("Caja").ValueField("nStbTipoPlazoPagosID")

            'Descripción:
            ObjCajadr.sDescripcionCaja = XdsCaja("Caja").ValueField("sDescripcionCaja")

            ObjCajadr.nCerrada = 0
            ObjCajadr.SetFieldNull("dFechaCierre")

            'Ubicación Caja:
            ObjCajadr.nStbBarrioID = XdsCaja("Caja").ValueField("nStbBarrioID")
            ObjCajadr.nStbMercadoID = XdsCaja("Caja").ValueField("nStbMercadoID")
            ObjCajadr.nStbPersonaLugarPagosID = XdsCaja("Caja").ValueField("nStbPersonaLugarPagosID")
            ...
            If Not ObjTmpCaja("Caja").ValueField("maxCodigo") Is DBNull.Value Then
                ObjCajadr.nCodigo = Format(ObjTmpCaja("Caja").ValueField("maxCodigo") + 1) ', "00"
            Else
                ObjCajadr.nCodigo = "1"
            End If
            ...
            'Y se posiciona sobre el registro modificado.
            XdtCaja.SetCurrentByID("nSteCajaID", ObjCajadr.nSteCajaID)
            'Refresca posicionamiento del Grid:
            Me.grdCaja.Row = XdtCaja.BindingSource.Position
    ...
    Private Sub Seguridad()
        Try
            ...
            'Reabrir una caja:
            If Seg.HasPermission("ReabrirCaja") = False Then
                Me.tbCaja.Items("toolReabrir").Enabled = False
            Else  'Habilita
                Me.tbCaja.Items("toolReabrir").Enabled = True
            End If

            'Reabrir todas las cajas:
            If Seg.HasPermission("ReabrirCajaTodas") = False Then
                Me.tbCaja.Items("toolReabrirTodas").Enabled = False
            Else  'Habilita
                Me.tbCaja.Items("toolReabrirTodas").Enabled = True
            End If
