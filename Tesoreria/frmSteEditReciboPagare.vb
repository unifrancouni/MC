...
Public Class frmSteEditReciboPagare
    ...
    Private Sub CargarRecibo()
        Try
            ...
            If Not ObjRecibodr.IsFieldNull("nReciboPagoNomina") Then
                Me.ckbPagoNómina.Checked = True
            Else
                Me.ckbPagoNómina.Checked = False
            End If
    ...
    Private Function ValidaDatosEntrada() As Boolean
        ...
        Try
            ...
            If Me.ckbPagoNómina.Checked() Then
                Dim xdtRecibosAsociadosAMinuta As BOSistema.Win.XDataSet.xDataTable
                If (Me.cneValor.Value > Me.cboMinuta.Columns("nMontoDeposito").Value) Then
                    MsgBox("Monto de la Minuta DEBE SER MAYOR O IGUAL" & Chr(13) & " que el Valor del Recibo.", MsgBoxStyle.Critical, NombreSistema)
                    ValidaDatosEntrada = False
                    Me.cmdAceptar.Enabled = True
                    Me.cmdCancelar.Enabled = True
                    Me.errRecibo.SetError(Me.cboMinuta, "Debe seleccionar una Minuta de Depósito válida.")
                ...
                SQLQuery = "SELECT        SUM(nValor) AS nValor" &
               " FROM dbo.SteReciboPagare INNER JOIN dbo.StbValorCatalogo ON dbo.SteReciboPagare.nStbEstadoReciboID = dbo.StbValorCatalogo.nStbValorCatalogoID" &
               " WHERE(nSteMinutaDepositoID = " & Me.cboMinuta.Columns("nSteMinutaDepositoID").Value & ")" &
               " AND (dbo.StbValorCatalogo.sCodigoInterno = '1')" &
                " GROUP BY nSteMinutaDepositoID"
                xdtRecibosAsociadosAMinuta = New BOSistema.Win.XDataSet.xDataTable

                xdtRecibosAsociadosAMinuta.ExecuteSql(SQLQuery)
                If xdtRecibosAsociadosAMinuta.Count > 0 Then
                    If (Me.cneValor.Value > (Me.cboMinuta.Columns("nMontoDeposito").Value) - xdtRecibosAsociadosAMinuta.ValueField("nvalor")) Then
                        MsgBox("Monto del Recibo no debe ser Mayor al saldo disponible para la Minuta" & Chr(13) & "Saldo disponible: " & (Me.cboMinuta.Columns("nMontoDeposito").Value) - xdtRecibosAsociadosAMinuta.ValueField("nvalor"), MsgBoxStyle.Critical, NombreSistema)
                        ValidaDatosEntrada = False
                        Me.cmdAceptar.Enabled = True
                        Me.cmdCancelar.Enabled = True
                        Me.errRecibo.SetError(Me.cboMinuta, "Debe seleccionar una Minuta de Depósito válida.")
            ...
            Else
                ...
                If RegistrosAsociados(strSQL) Then
                    ...
                    Exit Function
                End If
                'finalizacion de if pagonimina cheked
            End If
    ...
    Private Sub SalvarRecibo()
        ...
        Try
            ...
            Dim nPagoNomina As Integer
            dFechaRecibo = Me.cdeFecha.Value
			
            If ModoFrm <> "ADD" Then
                GuardarAuditoriaTablas(22, 2, "Modificar Recibo Pagaré", IdSteReciboPagare, InfoSistema.IDCuenta)
            End If
            ...
            'Else
            strSQL = " EXEC spSteGrabarReciboPagare " & Me.IdSteReciboPagare & "," & Me.IdStePagare & ",'" & Format(dFechaRecibo.Date, "yyyy-MM-dd") & "'," & Me.cneValor.Value & ",'" & Me.txtConcepto.Text & "'," & Me.IdCajero & "," & XdsRecibo("Minuta").ValueField("nSteMinutaDepositoID") & ",'" & Me.ModoFrm & "'," & InfoSistema.IDCuenta & "," & nPagoNomina
            'End If
            ...
            Else
                If ModoFrm = "ADD" Then
                    ...
                    GuardarAuditoriaTablas(22, 1, "Agregar Recibo Pagaré", IdSteReciboPagare, InfoSistema.IDCuenta)
