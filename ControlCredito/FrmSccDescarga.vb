...
Public Class FrmSccDescarga
            ...
            If intCaja = 0 Then
                ...
                Else
                    ...
                    Strsql = "SELECT     dbo.SteCaja.nSteCajaID, dbo.SteCaja.sDescripcionCaja,1 As Orden   " &
                            ...
                            " WHERE (dbo.StbDelegacionPrograma.nStbDelegacionProgramaID = " & InfoSistema.IDDelegacion &
                            "))  AND (ma.nActivo = 1)) AND (nCerrada = 0) "
                End If
    ...
    Private Sub CargarGenerados()
        Try
            ...
            Strsql = " SELECT dbo.SttProcesarEnvioES.nSteCajaid, dbo.SteCaja.sDescripcionCaja,    dbo.SttProcesarEnvioES.NoEnvio,   " &
                     " dbo.SttProcesarEnvioES.EstadoEnvio, " &
                     "CASE WHEN NoEnvio = 0 AND EstadoEnvio = 0 THEN 'Carga Inicial Generada' WHEN NoEnvio = 0 AND " &
                     " EstadoEnvio = 1 THEN 'Carga Inicial Enviada' WHEN NoEnvio = 0 AND EstadoEnvio = 2 THEN 'Carga Inicial Recepcionada' WHEN NoEnvio <> 0 AND " &
                     " EstadoEnvio = 0 THEN 'Generada' WHEN NoEnvio <> 0 AND EstadoEnvio = 1 THEN ' Enviada' WHEN NoEnvio <> 0 AND " &
                     " EstadoEnvio = 2 THEN 'Recepcionada En Central' END AS DesEstadoEnvio," &
                     "  dbo.SttProcesarEnvioES.FechaGenera, dbo.SsgCuenta.Login " &
                     " FROM dbo.SttProcesarEnvioES INNER JOIN " &
                     " dbo.SteCaja ON dbo.SttProcesarEnvioES.nSteCajaId = dbo.SteCaja.nSteCajaID INNER JOIN " &
                     " dbo.SsgCuenta ON dbo.SttProcesarEnvioES.SsgCuentaID = dbo.SsgCuenta.SsgCuentaID " &
                     " Where dbo.SttProcesarEnvioES.nSteCajaId =  " & Me.cboCaja.Columns("nSteCajaId").Value &
                ...
                For Columna = 0 To dsFuente.Tables(0).Columns.Count - 1
                    ...
                    If ColumnaNum = 0 Then 'Or ColumnaNum = TotalColumnas Then
                        ...
                        If dsFuente.Tables(0).Rows(Fila).Item(ColumnaNum).GetType().ToString = "System.DateTime" Then
                            oRead.Write(Format(dsFuente.Tables(0).Rows(Fila).Item(ColumnaNum), "yyyy-MM-dd HH:mm:ss") & "." &
                    ...
                    Else
                        ...
                        If dsFuente.Tables(0).Rows(Fila).Item(ColumnaNum).GetType().ToString = "System.DateTime" Then
                            oRead.Write(Trim(Delimitador) & Format(dsFuente.Tables(0).Rows(Fila).Item(ColumnaNum), "yyyy-MM-dd HH:mm:ss") & "." &
                            PadRight(Format(dsFuente.Tables(0).Rows(Fila).Item(ColumnaNum), "fff"), 3, "0"))
    ...
    Private Sub cmbProcesar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbProcesar.Click
        ...
        Dim dsEquivalente As New DataSet
        ...
        Dim xMunicipioAtendido As Integer
        ...
        Try
            ...
            ''agregar el and si la caja pertenece la atiende otro municipio
            xMunicipioAtendido = XComando.ExecuteScalar(" SELECT count (vwSteCaja.nStbMunicipioID) FROM vwSteCaja INNER JOIN StbMunicipioAtendidoDepartamentoAlterno ON vwSteCaja.nStbMunicipioID = StbMunicipioAtendidoDepartamentoAlterno.nStbMunicipioID WHERE nSteCajaID =" & Me.cboCaja.Columns("nSteCajaId").Value)
            If xMunicipioAtendido > 0 Then
                mySelectQuery = "Select * From SttProcesarTablasES Where nProcesar in (1,2) And nTipoProcesoExportar in (1,2)   And nProcesarCentralImportar=0" & IIf(XCajaId <> 0, " And nProcesarLocalExportar= 1 ", "")
                mySelectQueryCount = "Select Count(*) AS TotalT From SttProcesarTablasES Where nProcesar in (1,2) And nTipoProcesoExportar in (1,2)   And nProcesarCentralImportar=0 " & IIf(XCajaId <> 0, " And nProcesarLocalExportar= 1 ", "")
            ...
            If XCajaId = 0 Then 'Central solo puede exportar si ya se hizo el proceso de aplicacion de recibos
                ...
                If XNumEnvio > 0 Then ' Si es envio mayor que cero validar que existan los cierres para los cajeros fechas de la caja.
                    ...
                    'StrSqlx = "Select  nSttCentralArqueoRecuperacionID, dFechaArqueo, CASE WHEN nObligado = 1 THEN 'SI' ELSE 'NO' END AS DesObligado, nObligado FROM         dbo.SttCentralArqueoRecuperacion  WHERE     (EnvioNumero = " & XNumEnvio & ") AND (nSteCajaID = " & Me.cboCaja.Columns("nSteCajaId").Value & ")"

                    'DsArqNO = Me.GetDataSet(StrSqlx, myConnectionString, SegundosConexion)
                    'If DsArqNO.Tables(0).Rows.Count > 0 Then
                    ...
                    SqlCad = "SELECT     ListaCajerosLocal.nSteCajaIDLocal, ListaCajerosLocal.nSrhEmpleadoID, ListaCajerosLocal.sNombre, ListaCajerosLocal.dFechaRecibo " &
                             "    FROM         (SELECT     dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.SrhEmpleado.nSrhEmpleadoID, dbo.vwStbPersona.sNombre, " &
                             " dbo.SttLocalSccReciboOficialCaja.dFechaRecibo, SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nPrincipal) AS nPrincipal,  " &
                             "                     SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nMantenimientoValor) AS nMantenimientoValor, " &
                             "                     SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nInteresesCorrientes) AS nInteresesCorrientes, " &
                             "                     SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nInteresesMoratorios) AS nInteresesMoratorios, " &
                             "                     SUM(dbo.SttLocalSccReciboOficialCaja.nValor) AS nValor " &
                             " FROM          dbo.SttLocalSccReciboOficialCaja INNER JOIN    dbo.SttLocalSccReciboOficialCajaDetalle ON  " &
                             " dbo.SttLocalSccReciboOficialCaja.nSccReciboOficialCajaID = dbo.SttLocalSccReciboOficialCajaDetalle.nSccReciboOficialCajaID INNER JOIN " &
                             " dbo.SrhEmpleado ON dbo.SttLocalSccReciboOficialCaja.nSrhEmpleadoCajeroID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN " &
                             " dbo.vwStbPersona ON dbo.SrhEmpleado.nStbPersonaID = dbo.vwStbPersona.nStbPersonaID " &
                             " WHERE      (dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal = " & Me.cboCaja.Columns("nSteCajaId").Value & ") AND (dbo.SttLocalSccReciboOficialCaja.EnvioNumero = " & XNumEnvio & ") " &
                             " GROUP BY dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.vwStbPersona.sNombre, dbo.SrhEmpleado.nSrhEmpleadoID,  " &
                             " dbo.SttLocalSccReciboOficialCaja.dFechaRecibo) AS ListaCajerosLocal LEFT OUTER JOIN  (SELECT     SrhEmpleado_1.nSrhEmpleadoID, dbo.SteArqueoCaja.dFechaArqueo, dbo.SteArqueoCaja.nSteCajaID, dbo.SteCaja.sDescripcionCaja, " &
                             " dbo.StbValorCatalogo.sCodigoInterno, dbo.StbValorCatalogo.sDescripcion " &
                             "   FROM          dbo.StbPersona INNER JOIN " &
                             "   dbo.SrhEmpleado AS SrhEmpleado_1 ON dbo.StbPersona.nStbPersonaID = SrhEmpleado_1.nStbPersonaID INNER JOIN " &
                             " dbo.SteCajero ON SrhEmpleado_1.nSrhEmpleadoID = dbo.SteCajero.nSrhEmpleadoID INNER JOIN " &
                             " dbo.SteArqueoCaja ON SrhEmpleado_1.nSrhEmpleadoID = dbo.SteArqueoCaja.nSrhCajeroId INNER JOIN " &
                             " dbo.SteCaja ON dbo.SteArqueoCaja.nSteCajaID = dbo.SteCaja.nSteCajaID INNER JOIN " &
                             " dbo.StbValorCatalogo ON dbo.SteArqueoCaja.nStbEstadoArqueoID = dbo.StbValorCatalogo.nStbValorCatalogoID " &
                             "   WHERE      (dbo.StbValorCatalogo.sCodigoInterno = '2')) AS ArqueoCerrado ON  " &
                             " ListaCajerosLocal.nSrhEmpleadoID = ArqueoCerrado.nSrhEmpleadoID AND ListaCajerosLocal.nSteCajaIDLocal = ArqueoCerrado.nSteCajaID AND " &
                    ...
                    If DsBusca.Tables(0).Rows.Count > 0 Then
                        ...
                        Exit Sub
                    Else
                        SqlCad = " SELECT     SttCentralArqueoRecuperacion_1.dFechaArqueo, ListaArqueoFn.TotalCajeros, SttCentralArqueoRecuperacion_1.nObligado " &
                                    " FROM         (SELECT     nSteCajaID, EnvioNumero, dFechaArqueo, COUNT(nSrhCajeroId) AS TotalCajeros " &
                                    "  FROM          (SELECT     ArqueoTodos.nSrhCajeroId, ArqueoTodos.nSteCajaID, ArqueoTodos.dFechaArqueo, ArqueoTodos.nSteArqueoCajaID,  " &
                                                                           "  ArqueoTodos.EnvioNumero, ArqueoNormal.nSteArqueoCajaID AS Expr1 " &
                                                      " FROM          (SELECT     TOP (100) PERCENT dbo.StbValorCatalogo.sCodigoInterno, dbo.StbValorCatalogo.sDescripcion, " &
                                                                                                     " dbo.SteArqueoCaja.nSrhCajeroId, dbo.SteArqueoCaja.nSteCajaID, dbo.SteArqueoCaja.dFechaArqueo, " &
                                                                                                     " dbo.SteArqueoCaja.nSteArqueoCajaID, dbo.SttCentralArqueoRecuperacion.EnvioNumero, " &
                                                        "  dbo.SttCentralArqueoRecuperacion.nObligado FROM          dbo.SteArqueoCaja INNER JOIN dbo.StbValorCatalogo ON                                                                                             dbo.SteArqueoCaja.nStbEstadoArqueoID = dbo.StbValorCatalogo.nStbValorCatalogoID INNER JOIN " &
                                                        " dbo.SttCentralArqueoRecuperacion ON  dbo.SteArqueoCaja.nSteCajaID = dbo.SttCentralArqueoRecuperacion.nSteCajaID AND  dbo.SteArqueoCaja.dFechaArqueo = dbo.SttCentralArqueoRecuperacion.dFechaArqueo  WHERE      (dbo.StbValorCatalogo.sCodigoInterno = '2') " &
                                                        "AND (dbo.SteArqueoCaja.nSteCajaID = " & Me.cboCaja.Columns("nSteCajaId").Value & ") AND  (dbo.SttCentralArqueoRecuperacion.EnvioNumero = " & XNumEnvio & ") AND (dbo.SttCentralArqueoRecuperacion.nObligado = 1) " &
                                                                             " ORDER BY dbo.SteArqueoCaja.dFechaArqueo DESC) AS ArqueoTodos LEFT OUTER JOIN " &
                                                                               "  (SELECT     LIstaRec.nSteCajaIDLocal, LIstaRec.nSrhEmpleadoCajeroID, LIstaRec.dFechaRecibo, Arqueo.nSrhCajeroId,  " &
                                                       " Arqueo.nSteArqueoCajaID FROM          (SELECT     nSteCajaIDLocal, nSrhEmpleadoCajeroID, dFechaRecibo  FROM dbo.SttLocalSccReciboOficialCaja " &
                                                       " WHERE      (nSteCajaIDLocal = " & Me.cboCaja.Columns("nSteCajaId").Value & ") AND (EnvioNumero = " & XNumEnvio & ") " &
                                                       " GROUP BY nSrhEmpleadoCajeroID, dFechaRecibo, nSteCajaIDLocal) AS LIstaRec INNER JOIN  (SELECT     TOP (100) PERCENT StbValorCatalogo_1.sCodigoInterno, StbValorCatalogo_1.sDescripcion, SteArqueoCaja_1.nSrhCajeroId, SteArqueoCaja_1.nSteCajaID, SteArqueoCaja_1.dFechaArqueo,SteArqueoCaja_1.nSteArqueoCajaID FROM          dbo.SteArqueoCaja AS SteArqueoCaja_1 INNER JOIN  dbo.StbValorCatalogo AS StbValorCatalogo_1 ON  SteArqueoCaja_1.nStbEstadoArqueoID = StbValorCatalogo_1.nStbValorCatalogoID WHERE      (StbValorCatalogo_1.sCodigoInterno = '2') " &
                                                       "AND (SteArqueoCaja_1.nSteCajaID = " & Me.cboCaja.Columns("nSteCajaId").Value & ") " &
                                                       " ORDER BY SteArqueoCaja_1.dFechaArqueo DESC) AS Arqueo ON  LIstaRec.nSteCajaIDLocal = Arqueo.nSteCajaID AND LIstaRec.dFechaRecibo = Arqueo.dFechaArqueo AND                                                                                                    LIstaRec.nSrhEmpleadoCajeroID = Arqueo.nSrhCajeroId) AS ArqueoNormal ON " &
                ...
                SqlCad = "SELECT     ListaFi.nSteArqueoCajaID, dbo.vwSteRecibosArqueadosNoIngresados.nCodigo, dbo.vwSteRecibosArqueadosNoIngresados.NoRecibo,                      dbo.vwSteRecibosArqueadosNoIngresados.dFechaArqueo " &
                         " FROM         (SELECT     nSteArqueoCajaID " &
                       " FROM          (SELECT     TOP (100) PERCENT ArqueoCerrado.nSteArqueoCajaID " &
                                             "  FROM          (SELECT     dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.SrhEmpleado.nSrhEmpleadoID, dbo.vwStbPersona.sNombre, " &
                " dbo.SttLocalSccReciboOficialCaja.dFechaRecibo, SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nPrincipal) " &
                " AS nPrincipal, SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nMantenimientoValor) AS nMantenimientoValor, " &
                " SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nInteresesCorrientes) AS nInteresesCorrientes,  " &
                " SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nInteresesMoratorios) AS nInteresesMoratorios,  " &
                " SUM(dbo.SttLocalSccReciboOficialCaja.nValor) AS nValor " &
                " FROM          dbo.SttLocalSccReciboOficialCaja INNER JOIN " &
                " dbo.SttLocalSccReciboOficialCajaDetalle ON  " &
                " dbo.SttLocalSccReciboOficialCaja.nSccReciboOficialCajaID = dbo.SttLocalSccReciboOficialCajaDetalle.nSccReciboOficialCajaID INNER Join " &
                " dbo.SrhEmpleado ON dbo.SttLocalSccReciboOficialCaja.nSrhEmpleadoCajeroID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN dbo.vwStbPersona ON dbo.SrhEmpleado.nStbPersonaID = dbo.vwStbPersona.nStbPersonaID " &
                " WHERE      (dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal = " & Me.cboCaja.Columns("nSteCajaId").Value & ") AND (dbo.SttLocalSccReciboOficialCaja.EnvioNumero = " & XNumEnvio & ") " &
                " GROUP BY dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.vwStbPersona.sNombre,dbo.SrhEmpleado.nSrhEmpleadoID, dbo.SttLocalSccReciboOficialCaja.dFechaRecibo)  AS ListaCajerosLocal INNER JOIN  (SELECT     SrhEmpleado_1.nSrhEmpleadoID, dbo.SteArqueoCaja.dFechaArqueo, dbo.SteArqueoCaja.nSteCajaID, " &
                " dbo.SteCaja.sDescripcionCaja, dbo.StbValorCatalogo.sCodigoInterno, dbo.StbValorCatalogo.sDescripcion, dbo.SteArqueoCaja.nSteArqueoCajaID  FROM          dbo.StbPersona INNER JOIN dbo.SrhEmpleado AS SrhEmpleado_1 ON  dbo.StbPersona.nStbPersonaID = SrhEmpleado_1.nStbPersonaID INNER JOIN " &
                " dbo.SteCajero ON SrhEmpleado_1.nSrhEmpleadoID = dbo.SteCajero.nSrhEmpleadoID INNER JOIN dbo.SteArqueoCaja ON SrhEmpleado_1.nSrhEmpleadoID = dbo.SteArqueoCaja.nSrhCajeroId INNER JOIN dbo.SteCaja ON dbo.SteArqueoCaja.nSteCajaID = dbo.SteCaja.nSteCajaID INNER JOIN dbo.StbValorCatalogo ON " &
                " dbo.SteArqueoCaja.nStbEstadoArqueoID = dbo.StbValorCatalogo.nStbValorCatalogoID WHERE      (dbo.StbValorCatalogo.sCodigoInterno = '2')) AS ArqueoCerrado ON   ListaCajerosLocal.nSrhEmpleadoID = ArqueoCerrado.nSrhEmpleadoID AND  ListaCajerosLocal.nSteCajaIDLocal = ArqueoCerrado.nSteCajaID AND " &
                ...
                If XNumEnvio > 0 Then
                    ...
                    SqlCad = "  SELECT     UltimoArqueoRevisado.nSteArqueoCajaID, UltimoArqueoRevisado.nSteCajaIDLocal, SteArqueoCaja_1.nSteCajaID, " &
                                            " SteArqueoCaja_1.nSteArqueoCajaID AS nSteArqueoCajaIDPosterior, dbo.vwSteRecibosArqueadosNoIngresados.Serie, " &
                                          " dbo.vwSteRecibosArqueadosNoIngresados.sNombreColor, dbo.vwSteRecibosArqueadosNoIngresados.Departamento, " &
                                          " dbo.vwSteRecibosArqueadosNoIngresados.Municipio, dbo.vwSteRecibosArqueadosNoIngresados.nCodigo, " &
                                          " dbo.vwSteRecibosArqueadosNoIngresados.dFechaArqueo, dbo.vwSteRecibosArqueadosNoIngresados.NoRecibo, " &
                    " dbo.vwSteRecibosArqueadosNoIngresados.nCodigoTalonario " &
                   " FROM         (SELECT     TOP (1) ArqueoCerrado.nSteArqueoCajaID, ListaCajerosLocal.nSteCajaIDLocal " &
                                          " FROM          (SELECT     dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.SrhEmpleado.nSrhEmpleadoID, dbo.vwStbPersona.sNombre,  " &
                                                                                        "  dbo.SttLocalSccReciboOficialCaja.dFechaRecibo, SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nPrincipal) AS nPrincipal, " &
                                                                                         " SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nMantenimientoValor) AS nMantenimientoValor, " &
                                                                                         " SUM(dbo.SttLocalSccReciboOficialCajaDetalle.nInteresesCorrientes) AS nInteresesCorrientes, " &
                                                                 ...
                                                                 "  FROM          dbo.SttLocalSccReciboOficialCaja INNER JOIN " &
                                                                 "                         dbo.SttLocalSccReciboOficialCajaDetalle ON " &
                                                                                      "    dbo.SttLocalSccReciboOficialCaja.nSccReciboOficialCajaID = dbo.SttLocalSccReciboOficialCajaDetalle.nSccReciboOficialCajaID INNER Join  dbo.SrhEmpleado ON dbo.SttLocalSccReciboOficialCaja.nSrhEmpleadoCajeroID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN  dbo.vwStbPersona ON dbo.SrhEmpleado.nStbPersonaID = dbo.vwStbPersona.nStbPersonaID " &
                                                                 "  WHERE      (dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal = " & Me.cboCaja.Columns("nSteCajaId").Value & " ) AND (dbo.SttLocalSccReciboOficialCaja.EnvioNumero = " & XNumEnvio & ") " &
                                                                  " GROUP BY dbo.SttLocalSccReciboOficialCaja.nSteCajaIDLocal, dbo.vwStbPersona.sNombre, dbo.SrhEmpleado.nSrhEmpleadoID, " &
                                                                                       "   dbo.SttLocalSccReciboOficialCaja.dFechaRecibo) AS ListaCajerosLocal INNER JOIN " &
                                                                     " (SELECT     SrhEmpleado_1.nSrhEmpleadoID, dbo.SteArqueoCaja.dFechaArqueo, dbo.SteArqueoCaja.nSteCajaID, " &
                                                                                             "  dbo.SteCaja.sDescripcionCaja, dbo.StbValorCatalogo.sCodigoInterno, dbo.StbValorCatalogo.sDescripcion, " &
                                                       ...
                                                       " WHERE      (dbo.StbValorCatalogo.sCodigoInterno = '2')) AS ArqueoCerrado ON  ListaCajerosLocal.nSrhEmpleadoID = ArqueoCerrado.nSrhEmpleadoID AND ListaCajerosLocal.nSteCajaIDLocal = ArqueoCerrado.nSteCajaID AND  ListaCajerosLocal.dFechaRecibo = ArqueoCerrado.dFechaArqueo ORDER BY ArqueoCerrado.nSteArqueoCajaID) AS UltimoArqueoRevisado " &
                                                       "INNER JOIN  dbo.SteArqueoCaja AS SteArqueoCaja_1 ON UltimoArqueoRevisado.nSteArqueoCajaID < SteArqueoCaja_1.nSteArqueoCajaID INNER JOIN  dbo.vwSteRecibosArqueadosNoIngresados ON  SteArqueoCaja_1.nSteArqueoCajaID = dbo.vwSteRecibosArqueadosNoIngresados.nSteArqueoCajaID " &
                ...
                SqlCad = " SELECT     ISNULL(COUNT(*), 0) AS TotalCajeros " &
                                       " FROM         (SELECT     SsgCuentaID, SUM(mnuControlCredito) AS mnuControlCredito, SUM(MantExportarRecibosCajero) AS  MantExportarRecibosCajero" &
                                       " FROM          (SELECT     TOP (100) PERCENT dbo.SsgCuenta.SsgCuentaID, dbo.SsgAccion.CodInterno, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'mnuControlCredito' THEN 1 ELSE 0 END AS mnuControlCredito, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'MantExportarRecibosCajero' THEN 1 ELSE 0 END AS MantExportarRecibosCajero " &
                                       " FROM          dbo.vwSttExpSteCajaCajeros INNER JOIN " &
                                       " dbo.SteCajero ON dbo.vwSttExpSteCajaCajeros.nSteCajeroID = dbo.SteCajero.nSteCajeroID INNER JOIN " &
                                       " dbo.SrhEmpleado ON dbo.SteCajero.nSrhEmpleadoID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN " &
                                       " dbo.SsgCuenta ON dbo.SrhEmpleado.nSrhEmpleadoID = dbo.SsgCuenta.objEmpleadoID INNER JOIN " &
                                       " dbo.SsgCuentaRol ON dbo.SsgCuenta.SsgCuentaID = dbo.SsgCuentaRol.objCuentaID INNER JOIN " &
                                       " dbo.SsgRol ON dbo.SsgCuentaRol.objRolID = dbo.SsgRol.SsgRolID INNER JOIN " &
                                       " dbo.SsgRolAccion ON dbo.SsgRol.SsgRolID = dbo.SsgRolAccion.objRolID INNER JOIN " &
                                       " dbo.SsgAccion ON dbo.SsgRolAccion.objAccionID = dbo.SsgAccion.SsgAccionID " &
                                       " WHERE      (dbo.vwSttExpSteCajaCajeros.nSteCajaID IN " &
                                       " (SELECT     nSteCajaID   FROM dbo.vwSttExpFiltroLlaves " &
                                       " WHERE      (nSteCajaID = " & Me.cboCaja.Columns("nSteCajaId").Value & "))) " &
                                       " AND (dbo.vwSttExpSteCajaCajeros.nActiva = 1)" &
                                       " ORDER BY dbo.vwSttExpSteCajaCajeros.nSteCajeroID, dbo.SsgAccion.CodInterno) AS A " &
                ...
                SqlCad = " SELECT     ISNULL(COUNT(*), 0) AS TotalCajeros " &
                                       " FROM         (SELECT     SsgCuentaID, SUM(mnuControlCredito) AS mnuControlCredito, SUM(MantImportarInformacionCajero) AS  MantImportarInformacionCajero " &
                                       " FROM          (SELECT     TOP (100) PERCENT dbo.SsgCuenta.SsgCuentaID, dbo.SsgAccion.CodInterno, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'mnuControlCredito' THEN 1 ELSE 0 END AS mnuControlCredito, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'MantImportarInformacionCajero' THEN 1 ELSE 0 END AS MantImportarInformacionCajero " &
                                       " FROM          dbo.vwSttExpSteCajaCajeros INNER JOIN " &
                                       " dbo.SteCajero ON dbo.vwSttExpSteCajaCajeros.nSteCajeroID = dbo.SteCajero.nSteCajeroID INNER JOIN " &
                                       " dbo.SrhEmpleado ON dbo.SteCajero.nSrhEmpleadoID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN " &
                                       " dbo.SsgCuenta ON dbo.SrhEmpleado.nSrhEmpleadoID = dbo.SsgCuenta.objEmpleadoID INNER JOIN " &
                ...
                SqlCad = " SELECT     ISNULL(COUNT(*), 0) AS TotalCajeros " &
                                       " FROM         (SELECT     SsgCuentaID, SUM(mnuControlCredito) AS mnuControlCredito, SUM(AgregarReciboAutomatico) AS  AgregarReciboAutomatico ,SUM(BuscarGrupoSocia) AS  BuscarGrupoSocia " &
                                       " FROM          (SELECT     TOP (100) PERCENT dbo.SsgCuenta.SsgCuentaID, dbo.SsgAccion.CodInterno, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'mnuControlCredito' THEN 1 ELSE 0 END AS mnuControlCredito, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'AgregarReciboAutomatico' THEN 1 ELSE 0 END AS AgregarReciboAutomatico, " &
                                       " CASE WHEN dbo.SsgAccion.CodInterno = 'BuscarGrupoSocia' THEN 1 ELSE 0 END AS BuscarGrupoSocia " &
                                       " FROM          dbo.vwSttExpSteCajaCajeros INNER JOIN " &
                                       " dbo.SteCajero ON dbo.vwSttExpSteCajaCajeros.nSteCajeroID = dbo.SteCajero.nSteCajeroID INNER JOIN " &
                                       " dbo.SrhEmpleado ON dbo.SteCajero.nSrhEmpleadoID = dbo.SrhEmpleado.nSrhEmpleadoID INNER JOIN " &
                                       " dbo.SsgCuenta ON dbo.SrhEmpleado.nSrhEmpleadoID = dbo.SsgCuenta.objEmpleadoID INNER JOIN " &
            ...
            While lectorDatos.Read
                'If lectorDatos("sNombreTablaOrigen") = "SclSocia" Then
                '    MsgBox("scl socia")
       ...
       (myFileStream, System.Text.Encoding.Unicode)
            ...
            'Agregado para hacer la caja nueva del mismo lugar de pago.
            ' para lo del reinicio de recibos el 1ro de septiembre del 2014
            If XNumEnvio = 0 Then

                myXMLfile = CarpetaGenera & "\SttCajaEquivalente.Xml"
                myTXTfile = CarpetaGenera & "\SttCajaEquivalente.Txt"
                daCust = New SqlDataAdapter("Select Top 1 * From SttCajaEquivalente " & " Where nSteCajaId =" & Me.cboCaja.Columns("nSteCajaId").Value, con)
                daCust.Fill(dsEquivalente, "SttCajaEquivalente")
                'If EnviarXML = 1 Then
                If dsEquivalente.Tables(0).Rows.Count > 0 Then
               ...
               (myFileStream, System.Text.Encoding.Unicode)
                    ...
                    dsEquivalente.WriteXml(MyXmlTextWriter, XmlWriteMode.WriteSchema)
                    MyXmlTextWriter.Close()
                    myFileStream.Close()
                End If
