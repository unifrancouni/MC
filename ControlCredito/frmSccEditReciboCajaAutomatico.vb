...
'-------------------------------------------------------------------------
Public Class frmSccEditReciboCajaAutomatico
    ...
    Private Sub InicializarVariables()
        ...
        Try
            ...
            strSQL = " SELECT sNombreGrupo,sNombreSocia,sSerieDelegacion,nStbDepartamentoID,sCodigoTipoPlanNegocio, CodigoPrograma " & _
                     " FROM vwSccGrupoSociaChequeEmitido " & _
            ...
            nTipoPrograma = XdtDatos.ValueField("CodigoPrograma")
            ...
            If ModoFrm = "ADD" Then
                ...
                If IntRegistros > 0 Then
                    ...
                    nMoraCuota = CalcularMora(XdtDatos.ValueField("nCuota"), XdtDatos.ValueField("nPrincipal"), XdtDatos.ValueField("nMantenimientoValor"), dFechaPagoSinMora, nTipoPrograma)
                ...
                While IntRegistros > 0
                    If XdtDatos.ValueField("dFechaPago") < DateAdd(DateInterval.Day, 1, Me.cdeFecha.Value) Then
                        nMoraTotal = nMoraTotal + CalcularMora(XdtDatos.ValueField("nCuota"), XdtDatos.ValueField("nPrincipal"), XdtDatos.ValueField("nMantenimientoValor"), XdtDatos.ValueField("dFechaPago"), nTipoPrograma)
            ...
            'Determinar la Caja
            'strSQL = " SELECT a.nSteCajaID,b.nCodigo FROM SttProcesarParametroES a " & _
            '         " INNER JOIN SteCaja b " & _
            '         " ON a.nSteCajaID = b.nSteCajaID "
            ...
            'REM Esto es temporal mientras el parámtro lo dejan de cambiar para pruebas
            'IdCaja = 1
    ...
    Private Function ValidaDatosEntrada() As Boolean
        ...
        Try
            ...
            If nValorRecibo < CDbl(Me.txtMontoAdeudadoCuota.Text) Then
                MsgBox("El Valor del Recibo NO DEBE ser Menor que el Monto" & Chr(10) & _
                       "de la Primer Cuota Adeudada: " & Format(CDbl(Me.txtMontoAdeudadoCuota.Text), "#,##0.00"), MsgBoxStyle.Critical, "SMUSURA0")
                ValidaDatosEntrada = False
                Me.cmdAceptar.Enabled = True
                Me.cmdCancelar.Enabled = True
                Me.errRecibo.SetError(Me.cneValor, "El Valor del Recibo NO DEBE ser Menor que el Monto" & Chr(10) & _
                       "de la Primer Cuota Adeudada: " & Format(CDbl(Me.txtMontoAdeudadoCuota.Text), "#,##0.00"))
                Me.cneValor.Focus()
                Exit Function
            End If
            ...
            strSQL = " Select sValorParametro " & _
                             ...
                             " WHERE (nStbValorParametroID = 107) "

            XdtDatos.ExecuteSql(strSQL)

            If XdtDatos.Count > 0 Then
                ValidaMismoDia = XdtDatos.ValueField("sValorParametro")
            End If

            If ValidaMismoDia = 1 Then



                strSQL = " SELECT    dbo.SccReciboOficialCaja.dFechaRecibo FROM         dbo.SccReciboOficialCaja INNER JOIN  dbo.StbValorCatalogo ON dbo.SccReciboOficialCaja.nStbEstadoReciboID = dbo.StbValorCatalogo.nStbValorCatalogoID WHERE     (dbo.StbValorCatalogo.sCodigoInterno <> '3') AND (dbo.SccReciboOficialCaja.nSccSolicitudChequeID = " & Me.IdSccSolicitudCheque & ") AND  (DAY(dbo.SccReciboOficialCaja.dFechaRecibo) = " & dFechaRecibo.Date.Day & ") AND (MONTH(dbo.SccReciboOficialCaja.dFechaRecibo) = " & dFechaRecibo.Date.Month & ") AND  (YEAR(dbo.SccReciboOficialCaja.dFechaRecibo) = " & dFechaRecibo.Date.Year & " )"
                ...
                If XdtDatos.Count > 0 Then
                    ...
                    If MsgBox("Ya existe Recibo ingresado para esta socia en esa fecha," & Chr(10) & _
                                                "Esta seguro de ingresar este nuevo recibo.", MsgBoxStyle.YesNo, "SMUSURA0") = MsgBoxResult.No Then
                        ValidaDatosEntrada = False
                        Me.cmdAceptar.Enabled = True
                        Me.cmdCancelar.Enabled = True
                        Me.errRecibo.SetError(Me.cdeFecha, "Ya existe Recibo ingresado para esta socia en esa fecha")
                        Me.cdeFecha.Focus()
                        Exit Function
...
#End Region
    ...
    Private Function CalcularMora(ByVal nCuotaSinMora As Double, ByVal nPrincipalSinMora As Double, ByVal nMantValorSinMora As Double, ByVal dFechaPagoSinMora As Date, ByVal nTipoPrograma As Integer) As Double    'cdeFecha_ValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cdeFecha.ValueChanged
        Dim XdtDatos As New BOSistema.Win.XDataSet.xDataTable
        Try
            ...
            ''If nCodigoTipoPlanNegocio = 1 Or nCodigoTipoPlanNegocio = 2 Then
            If nTipoPrograma = 1 Then '' Usura Cero
                strSQL = " Select sValorParametro " & _
                         " FROM StbValorParametro " & _
                         " WHERE (nStbValorParametroID = 3) "
            Else  '' Ventana de genero
                If nTipoPrograma = 2 Then '' Ventana de Genero
                    strSQL = " Select sValorParametro " & _
                             " FROM StbValorParametro " & _
                             " WHERE (nStbValorParametroID = 50) "

                Else '' PDIBA
                    strSQL = " Select sValorParametro " & _
                             " FROM StbValorParametro " & _
                             " WHERE (nStbValorParametroID = 100) "
            ...
            End If
                ...
                If (dFechaRecibo.Date > dFechaPagoSinMora.Date) And (nPrincipalSinMora > 0) Then
                    'MsgBox(dFechaRecibo.Date)
                    'MsgBox(dFechaPagoSinMora.Date)
