...
'-------------------------------------------------------------------------
Public Class frmSccReciboCajaAutomatico
    ...
    Private Sub tbRecibo_ItemClicked(ByVal sender As Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tbRecibo.ItemClicked
        ...
        Dim strSQL As String
        If e.ClickedItem.Name <> "toolAyudaM" And e.ClickedItem.Name <> "toolModificarAutomatico" And e.ClickedItem.Name <> "toolImprimirM" And e.ClickedItem.Name <> "toolRefrescarM" Then
    ...
    Private Sub LlamaCorteTrasladoValores()
        Dim ObjFrmSteCierreTrasladoValores As New frmSteCierreTrasladoValores
        Dim XcDatos As New BOSistema.Win.XComando
        Try
            Dim strSQL As String
            ...
            ObjFrmSteCierreTrasladoValores.nSteCajaID = IdSteCaja
            ObjFrmSteCierreTrasladoValores.nSrhCajeroID = IdSrhCajero
            ObjFrmSteCierreTrasladoValores.MinimizeBox = False
            ObjFrmSteCierreTrasladoValores.MaximizeBox = False
            ObjFrmSteCierreTrasladoValores.StartPosition = FormStartPosition.CenterScreen
            'ObjFrmSteEditCierreTrasladoValores.sColorFrm = "Verde"
            ObjFrmSteCierreTrasladoValores.ShowDialog()

        Catch ex As Exception
            Control_Error(ex)
        Finally
            ObjFrmSteCierreTrasladoValores.Close()
            ObjFrmSteCierreTrasladoValores = Nothing
    ...
    Private Sub LlamaAgregarReciboAutomatico(ByVal IdEspecial As Short)
        ...
        Try
            ...
            ''strSQL = " SELECT     NoEnvio, nSteCajaId, EstadoEnvio, SsgCuentaID, FechaGenera, AplicadoEnCentral  FROM         dbo.SttProcesarEnvioES WHERE     (nSteCajaId = " & IdSteCaja & ") AND (NoEnvio > 0) AND (AplicadoEnCentral = 0) "
            'If xObligarConCajaLocal = 1 Then
            '    'strSQL = " SELECT     TOP (1) NoEnvio, nSteCajaId, EstadoEnvio, SsgCuentaID, FechaGenera, AplicadoEnCentral FROM         dbo.SttProcesarEnvioES WHERE     (nSteCajaId = " & IdSteCaja & ")  ORDER BY NoEnvio DESC"
            '    'XdtDatos.ExecuteSql(strSQL)

            '    'If XdtDatos.Count > 0 Then
            '    '    If XdtDatos.ValueField("NoEnvio") = 0 Or (XdtDatos.ValueField("NoEnvio") > 0 And XdtDatos.ValueField("AplicadoEnCentral") = 0) Then
            '    '        MsgBox("No puede Agregar Recibos. Por que se genero archivo para la base local." & Chr(13) & " Recuerde ingresar los recibos una vez aceptada la carga y antes de generar el siguiente envio.  ", MsgBoxStyle.Information)
            '    '        Exit Sub
            ...
            '        MsgBox("No puede Agregar Recibos. Por que Esa Ficha se encuentra en una base local enviada ." & Chr(13) & " Recuerde ingresar los recibos una vez aceptada la carga y antes de generar el siguiente envio.  ", MsgBoxStyle.Information)
            '        Exit Sub
    ...
    Private Sub LlamaEliminarRecibo(ByVal nOpcionAutomatico As Short)
        ...
        Try
            ...
            If xObligarConCajaLocal = 1 Then
                ...
                'If XdtDatos.Count > 0 Then
                '    If XdtDatos.ValueField("NoEnvio") = 0 Or (XdtDatos.ValueField("NoEnvio") > 0 And XdtDatos.ValueField("AplicadoEnCentral") = 0) Then
                '        MsgBox("No puede Anular  Recibos. Por que se genero archivo para la base local." & Chr(13) & " Recuerde anular los recibos una vez aceptada la carga y antes de generar el siguiente envio.  ", MsgBoxStyle.Information)
                '        Exit Sub
                ...
                If XdtDatos.Count > 0 Then
                    ...
                    MsgBox("No puede Anular Recibos. Por que Esa Ficha se encuentra en una base local enviada ." & Chr(13) & " Recuerde ingresar los recibos una vez aceptada la carga y antes de generar el siguiente envio.  ", MsgBoxStyle.Information)
                    Exit Sub
    ...
    Private Sub Seguridad()
        Try
            ...
            If Seg.HasPermission("AnularReciboAutomatico") Then
            ...
            'Corte Traslado de Valores
            If Seg.HasPermission("MantCierreTrasladoValor ") Then
                Me.toolCorteTrasladoValores.Enabled = True
            Else
                Me.toolCorteTrasladoValores.Enabled = False
            End If


            'Imprimir Reporte CC44
            If Seg.HasPermission("ImprimirReporteCC44") Then
                Me.toolReporteCC44.Enabled = True
            Else
                Me.toolReporteCC44.Enabled = False
            End If

            'Imprimir Reporte CC45
            If Seg.HasPermission("ImprimirReporteCC45") Then
                Me.toolReporteCC45.Enabled = True
            Else
                Me.toolReporteCC45.Enabled = False
            End If

            'Desglose de denominaciones
            If Seg.HasPermission("MantCajeroDenominacion") Then
                Me.toolArqueo.Enabled = True

            Else
                Me.toolArqueo.Enabled = False
            End If
    ...
    Private Sub toolReporteCC45_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles toolReporteCC45.Click
        LlamaImprimir(3)
    End Sub
    Private Sub LlamaImprimir(ByVal IntReporte As Integer)
        ...
        Try
            ...
            ObjFrmSteParametrosCajeros.PresentarCajeros = 1
            ObjFrmSteParametrosCajeros.NomRep = IntReporte
            ObjFrmSteParametrosCajeros.ColorVentana = "Verde"
    ...
    Private Sub toolReporteCC44_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles toolReporteCC44.Click
        LlamaImprimir(2)
    End Sub
    ...
    ' Autor:                
    ' Fecha:                29/10/2012
    ' Procedure Name:       ExistenArqueoIncompleto
    ' Descripción:          Ver si el cajero  tiene arqueos pendientes, para no permitir agregar recibos.
    '-------------------------------------------------------------------------
    Private Sub ExistenArqueoIncompleto()
        Dim XdtDatos As New BOSistema.Win.XDataSet.xDataTable
        Try
            Dim strSQL As String = ""
            ...
            'Ver parametros si existe para obligar al cajero a que no tenga arqueos sin definir

            strSQL = "SELECT     dbo.StbValorParametro.sValorParametro  FROM         dbo.StbParametro INNER JOIN  dbo.StbValorParametro ON dbo.StbParametro.nStbParametroID = dbo.StbValorParametro.nStbParametroID WHERE     (dbo.StbParametro.sDescripcionParametro = 'Obligar Cajero Arqueo No Abierto')"
            ...
            If XdtDatos.Count > 0 Then
                ...
                If XdtDatos.ValueField("sValorParametro") = "1" Then
                    ...
                    If XdtDatos.Count > 0 Then
                        EmpleadoId = XdtDatos.ValueField("objEmpleadoID")

                        strSQL = " SELECT     nSteCajaID, dFechaRecibo, sDescripcionCaja, nSrhEmpleadoCajeroID  FROM         dbo.vwSccCajerosCajasAutomaticosSinArqueos  " & _
                 "  WHERE     (nSrhEmpleadoCajeroID = " & EmpleadoId & " )  "

                        XdtDatos.ExecuteSql(strSQL)
                        If XdtDatos.Count > 0 Then

                            MsgBox("Tiene Arqueos no ingresados para recibos automáticos en la caja " & XdtDatos.ValueField("sDescripcionCaja") & " Con Fecha:" & XdtDatos.ValueField("dFechaRecibo"), MsgBoxStyle.Critical, "SMUSURA0")
                            toolAgregarAutomatico.Enabled = False
                            toolModificarAutomatico.Enabled = False
                            toolEliminarAutomatico.Enabled = False
                            toolRefrescarM.Enabled = False
                            toolImprimirM.Enabled = False
                            toolCorteTrasladoValores.Enabled = False

                        End If
                    Else
                        MsgBox("No se obtuvo la informacion del cajero/empleado" & XdtDatos.ValueField("sDescripcionCaja") & " Con Fecha:" & XdtDatos.ValueField("dFechaRecibo"), MsgBoxStyle.Critical, "SMUSURA0")
    ...
    Private Sub toolArqueo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles toolArqueo.Click
        Dim objFrm As New frmSteEditArqueoEfectivo
        Try
