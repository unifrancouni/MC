...
'-------------------------------------------------------------------------
Public Class frmSccEditReciboCajaManual
    ...
    Private Sub ObtenerSiModifica()
        ...
        Try
            ...
            'Strsql = " SELECT a.nStbValorCatalogoID FROM StbValorCatalogo a INNER JOIN StbCatalogo b ON a.nStbCatalogoID = b.nStbCatalogoID WHERE a.sCodigoInterno = '1' And b.sNombre = 'EstadoReciboOficialCaja' "

            'If ObjRecibodr.nStbEstadoReciboID <> XcDatos.ExecuteScalar(Strsql) Then
            '    blnModificar = False
            '    Exit Sub
            'End If
            ...
            '         " WHERE convert(datetime,dFechaRecibo,105) > convert(datetime,'" & ObjRecibodr.dFechaRecibo & "',105)" & _
            ...
            '         " AND nSccSolicitudChequeID = " & Me.IdSccSolicitudCheque
            ...
            '         " Where nSccReciboOficialCajaID = " & Me.IdReciboOficialCaja
    ...
    Private Sub InicializarVariables()
        ...
        Try
            ...
            If ModoFrm = "ADD" Then
                ...
                'PRIMERA VERSION
                'strSQL = " SELECT a.dFechaPago,a.nCuota - isnull(b.nPrincipal,0) - isnull(b.nInteresesCorrientes,0) - isnull(b.nMantenimientoValor,0) as nCuota," & _
                '         " a.nAmortizacion - isnull(b.nPrincipal,0) as nPrincipal, " & _
                '         " a.nMantValor - isnull(b.nMantenimientoValor,0) as nMantenimientoValor, " & _
                '         " a.nNumCuota FROM (SELECT * FROM vwSccDatosTablaAmortizacion WHERE sCodEstadoPago = '1' AND nSccSolicitudChequeID = " & Me.IdSccSolicitudCheque & ") a " & _
                '         " LEFT OUTER JOIN (Select * FROM vwSccTablaPagos ) b " & _
                '         " ON a.nSccTablaAmortizacionID = b.nSccTablaAmortizacionID " & _
                '         " WHERE (b.nSccReciboOficialCajaID is Null OR (a.nAmortizacion - b.nPrincipal) > 0) " & _
                '         " AND convert(datetime,a.dFechaPago,105) < convert(datetime,'" & FechaServer.Date & "',105)+1" & _
                '         " ORDER BY a.dFechaPago"
                ...
                '    If XdtDatos.ValueField("dFechaPago") < DateAdd(DateInterval.Day, 1, Me.cdeFecha.Value) Then
                '        nMontoAdeudadoTotal = nMontoAdeudadoTotal + XdtDatos.ValueField("nCuota")
                '        nMoraTotal = nMoraTotal + CalcularMora(XdtDatos.ValueField("nCuota"), XdtDatos.ValueField("nPrincipal"), XdtDatos.ValueField("nMantenimientoValor"), XdtDatos.ValueField("dFechaPago"))
                '    End If
                '    XdtDatos.Source.MoveNext()
                '    IntRegistros = IntRegistros - 1
                'End While

                'Me.txtMontoAdeudado.Text = Format(nMontoAdeudadoTotal + nMoraTotal, "#,##0.00")
                'Me.txtMora.Text = Format(nMoraTotal, "#,##0.00")
                Me.cdeFecha.Value = dFechaPagoSinMora.Date

            Else
                Me.Text = "Modificar Recibo Oficial de Caja"

                'strSQL = " SELECT a.dFechaPago,a.nCuota - CASE WHEN b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & " THEN 0 ELSE isnull(b.nPrincipal,0) END - CASE WHEN b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & " THEN 0 ELSE isnull(b.nInteresesCorrientes,0) END - CASE WHEN b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & " THEN 0 ELSE isnull(b.nMantenimientoValor,0) END as nCuota," & _
                '         " a.nAmortizacion - CASE WHEN b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & " THEN 0 ELSE isnull(b.nPrincipal,0) END as nPrincipal, " & _
                '         " a.nMantValor - CASE WHEN b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & " THEN 0 ELSE isnull(b.nMantenimientoValor,0) END as nMantenimientoValor, " & _
                '         " a.nNumCuota FROM vwSccDatosTablaAmortizacion a " & _
                '         " LEFT OUTER JOIN vwSccTablaAmortizacionPagos b " & _
                '         " ON a.nSccTablaAmortizacionID = b.nSccTablaAmortizacionID " & _
                '         " WHERE b.nSccReciboOficialCajaID = " & Me.IdSccReciboOficialCaja & _
                '         " ORDER BY a.dFechaPago"
            ...
            'Me.toolAgregarOC.Visible = False
            'Me.tbDesglosePago.Visible = False
            ...
            'LO SUBI
            'strSQL = "SELECT sNombreGrupo,sNombreSocia,sSerieDelegacion,nSclFichaNotificacionID FROM vwSccGrupoSociaChequeEmitido " & _
            '         "WHERE nSccSolicitudChequeID = " & Me.IdSccSolicitudCheque
    ...
    Private Function ValidaDatosEntrada() As Boolean
        ...
        Try
            ...
            'Determinar duplicados en el Número del Recibo
          ...
          " WHERE (nStbValorParametroID = 114) "
            ...
            '' Meno A la fecha de corte se considera talonario penultimo 
            '' si no se considera el ultimo tipo
            ...
            Else
                If ModoFrm = "UPD" Then
                    'ObjTmpRecibo.Filter = " nCodigo = " & Me.txtNumRecibo.Text & " And nSccReciboOficialCajaID <> " & Me.IdSccReciboOficialCaja & " AND nStbEstadoReciboID NOT IN (SELECT a.nStbValorCatalogoID FROM StbValorCatalogo a INNER JOIN StbCatalogo b ON a.nStbCatalogoID = b.nStbCatalogoID WHERE a.sCodigoInterno = '3' And b.sNombre = 'EstadoReciboOficialCaja') "
                    '& " And sCodigoInterno <> '3'"   al final de la segunda linea los anulados.
                    strSQL = " SELECT nSccReciboOficialCajaID FROM vwSccReciboSerie " & _
                             " WHERE (nManual = 1) And  nCodigoTalonario=" & nCodigoTalonario & " And sCodTipoPlazoPago= '" & sCodigoPlazo & "'   And sSerieDelegacion = '" & Me.txtSerieDelegacion.Text & "' AND TipoPrograma = " & sTipoPrograma & "   AND nCodigo = " & Me.txtNumRecibo.Text & " And nSccReciboOficialCajaID <> " & Me.IdSccReciboOficialCaja & " And sCodigoInterno <> '3'" & _
                             "AND (dFechaRecibo > CONVERT(DATETIME, '" & Format(dFechaMinimo, "yyyy-MM-dd") & "', 102))"
                ...
                Else
                    ...
                    '& " And sCodigoInterno <> '3'"   al final de la segunda linea los anulados.
                    strSQL = " SELECT nSccReciboOficialCajaID FROM vwSccReciboSerie " & _
                             " WHERE (nManual = 1)  And  nCodigoTalonario=" & nCodigoTalonario & " And sCodTipoPlazoPago= '" & sCodigoPlazo & "'   And sSerieDelegacion = '" & Me.txtSerieDelegacion.Text & "' AND TipoPrograma = " & sTipoPrograma & " AND nCodigo = " & Me.txtNumRecibo.Text & " And sCodigoInterno <> '3'" & _
                             "AND (dFechaRecibo > CONVERT(DATETIME, '" & Format(dFechaMinimo, "yyyy-MM-dd") & "', 102))"
                End If
            End If
            ...
            If XdtDatos.Count > 0 Then 'REVISAR  Antes era > 0 Esto es por lo de los recibos discontinuos  a partir del 1ro de enero del 2015

                'If XdtDatos.ValueField("Scodigo") > 2 Then  ''Es Ventana de genero

                'Else '' Es Usura cero con solo que exista 1
            ...
            If sFuente = "" Then
                ...
                ' Esto se utilizó para ingresar los recibos duplicados de Masaya
            ...
            '    MsgBox("No puede utilizarse la Fecha de Recibo ingresada," & Chr(10) & _
            '            "porque temporalmente No está habilitado el cálculo de la Mora.", MsgBoxStyle.Critical, "SMUSURA0")
            '    ValidaDatosEntrada = False
            '    If ModoFrm = "ADD" Then
            ...
            '    End If
            '    Me.errRecibo.SetError(Me.cdeFecha, "No puede utilizarse la Fecha de Recibo ingresada," & Chr(10) & _
            '            "porque temporalmente No está habilitado el cálculo de la Mora.")
            '    Me.cdeFecha.Focus()
            '    Exit Function
            'End If


            'Validar que el Valor del Recibo no sobrepase el valor de la última cuota
    ...
    Private Sub SalvarRecibo()
        ...
        Try
            ...
            'Else
            strSQL = " EXEC spSccGrabarReciboOficialCajaManual " & Me.IdSccReciboOficialCaja & "," & Me.IdSccSolicitudCheque & "," & Me.txtNumRecibo.Text & ",'" & Format(dFechaRecibo.Date, "yyyy-MM-dd") & "'," & Me.cneValor.Value.ToString().Replace(",", ".") & ",'" & Me.txtConcepto.Text & "'," & XdsRecibo("Representante").ValueField("nSrhEmpleadoID") & "," & IdMinutaDeposito & ",'" & Me.ModoFrm & "'," & InfoSistema.IDCuenta & "," & Me.IdEspecial & "," & IdCaja & "," & nCodigoTalonario
            'End If
...
#End Region
    ...
    Private Sub txtNumRecibo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtNumRecibo.TextChanged
        ...
        Try
            ...
            '    'Esto se utilizó para el ingreso de los recibos repetidos para Masaya
            '    'order by nSteMinutaDepositoID desc
