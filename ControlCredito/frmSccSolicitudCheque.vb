...
Public Class frmSccSolicitudCheque
    ...
    Private Sub FormatoSolicitudCheque()
        Try
            ...
            Me.grdDetalles.Splits(0).DisplayColumns("nSccTipoSolicitudChequeID").Visible = False
        ...
        Try
            ...
            If ModoAcc = "REV" Then
                ...
                If XdsSolicitudCheque("Solicitud").ValueField("nSccTipoSolicitudChequeID") = 3 Then
                    Dim nMontoCreditoCuentaBancaria As Double
                    StrSql = "SELECT  dbo.SccSolicitudChequeDetalle.nMonto FROM  dbo.SccSolicitudChequeDetalle INNER JOIN dbo.ScnCatalogoContable ON dbo.SccSolicitudChequeDetalle.nScnCatalogoContableID = dbo.ScnCatalogoContable.nScnCatalogoContableID INNER JOIN" &
                            "   dbo.SteCuentaBancaria ON dbo.ScnCatalogoContable.nSteCuentaBancariaID = dbo.SteCuentaBancaria.nSteCuentaBancariaID WHERE (nSccSolicitudChequeID = " & XdsSolicitudCheque("Solicitud").ValueField("nSccSolicitudChequeID") & ") AND (nDebito = 0)"
                    nMontoCreditoCuentaBancaria = XcDatos.ExecuteScalar(StrSql)
                    If nMontoCreditoCuentaBancaria <> XdsSolicitudCheque("Solicitud").ValueField("nMonto") And (XdsSolicitudCheque("Solicitud").ValueField("nSccTipoSolicitudChequeID") = 3) Then
                        MsgBox("Los movimientos contables no coinciden con el" & Chr(13) & "monto total de la Solicitud (C$ " & XdsSolicitudCheque("Solicitud").ValueField("nMonto") & ").", MsgBoxStyle.Information)
                        Exit Sub
                    End If
    ...
    Private Sub LlamaGenerarCheques()
        ...
        Try
            ...
            'Dim XcDatosT As BOSistema.Win.XDataSet.xDataTable
            'XcDatosT = New BOSistema.Win.XDataSet.xDataTable
            'Dim contad As Integer = 0
            'Dim CadTmp As String


            'CadTmp = "SELECT        c.nScnCatalogoContableID,c.sNombreCuenta, SUM(d.nMonto) AS nMonto FROM            dbo.SccSolicitudChequeDetalle AS d INNER JOIN                      dbo.ScnCatalogoContable AS c ON d.nScnCatalogoContableID = c.nScnCatalogoContableID INNER JOIN  dbo.SteImpresionCheque ON d.nSccSolicitudChequeID = dbo.SteImpresionCheque.nSccSolicitudChequeID WHERE        (d.nDebito = 0) AND (c.nSteCuentaBancariaID IS NOT NULL) AND (dbo.SteImpresionCheque.nUsuarioCreacionID = )" & InfoSistema.IDCuenta & " GROUP BY c.nScnCatalogoContableID, dbo.SteImpresionCheque.nUsuarioCreacionID, c.sCodigoCuenta, c.sNombreCuenta"
            'XcDatosT.ExecuteSql(CadTmp)
            'While contad < XcDatosT.Count


            '    If nMontoSaldoRojo(XcDatosT.ValueField("nScnCatalogoContableID"), 0, XcDatosT.ValueField("nMonto")) < 0 Then
            '        MsgBox("El movimiento crearía sobregiro bancario en " & XcDatosT.ValueField("nScnCatalogoContableID"), MsgBoxStyle.Critical, NombreSistema)
            '        Exit Sub
            '    End If

            '    XcDatosT.Source.MoveNext()
            '    contad = contad + 1
    ...
    Private Sub InsertarDetallesSolicitud()
        ...
        Dim XdtTmpParametrosSegundaFuente As BOSistema.Win.XDataSet.xDataTable
        XdtTmpParametrosSegundaFuente = New BOSistema.Win.XDataSet.xDataTable

        Dim xdtRegistrosDesembolsoSegundaFuente As BOSistema.Win.XDataSet.xDataTable
        xdtRegistrosDesembolsoSegundaFuente = New BOSistema.Win.XDataSet.xDataTable
        ...
        Try
            ...
            StrSql = "SELECT dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nSccSolicitudDesembolsoSegundaFuenteCreditoID,dbo.SccSolicitudDesembolsoCredito.nSccSolicitudDesembolsoCreditoID, dbo.SccSolicitudCheque.nSccSolicitudChequeID, dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionID" & _
                    " FROM dbo.SccSolicitudCheque INNER JOIN dbo.SccSolicitudDesembolsoCredito ON  dbo.SccSolicitudCheque.nSccSolicitudDesembolsoCreditoID = dbo.SccSolicitudDesembolsoCredito.nSccSolicitudDesembolsoCreditoID " & _
                    " INNER JOIN dbo.SclFichaNotificacionDetalle ON dbo.SccSolicitudDesembolsoCredito.nSclFichaNotificacionDetalleID = dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionDetalleID " & _
                    " INNER JOIN dbo.SccSolicitudDesembolsoSegundaFuenteCredito ON dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionDetalleID = dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nSclFichaNotificacionDetalleID" & _
                    " WHERE dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionID =" & XdsSolicitudCheque("Solicitud").ValueField("nSclFichaNotificacionID") & _
                    " AND dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nScnFuenteFinanciamientoSegundaFuenteID <> 0"

            xdtRegistrosDesembolsoSegundaFuente.ExecuteSql(StrSql)

            If xdtRegistrosDesembolsoSegundaFuente.Count > 0 Then

                StrSql = "SELECT TP.nScnFuenteFinanciamientoID, TP.nScnCatalogoContableID, TP.nDebito " & _
                    "FROM  ScnTransaccionParametrosSegundaFuente  TP INNER JOIN StbValorCatalogo CA ON TP.nStbTipoDocContableID = CA.nStbValorCatalogoID " & _
                ...
                XdtTmpParametrosSegundaFuente.ExecuteSql(StrSql)
                IntRegistros = XdtTmpParametrosSegundaFuente.Count
                ...
                While IntRegistros > 0
                    ...
                    StrSql = "Insert Into SccSolicitudChequeDetalle " & _
                                     ...
                                     " Values (" & XdsSolicitudCheque("Solicitud").ValueField("nSccSolicitudChequeID") & ", " & XdtTmpParametrosSegundaFuente.ValueField("nScnCatalogoContableID") & _
                                     ", " & XdtTmpParametrosSegundaFuente.ValueField("nDebito") & ", " & XdsSolicitudCheque("Solicitud").ValueField("nMonto") & ", " & InfoSistema.IDCuenta & "," & "GetDate())"


                    Trans.ExecuteSql(StrSql)
                    '---------------------------
                    XdtTmpParametrosSegundaFuente.Source.MoveNext()
            ...
            Else
                ...
                While IntRegistros > 0
                    ...
                    'Insertar Cuenta En los Detalles de Solicitud Con parametros Segunda Fuente
