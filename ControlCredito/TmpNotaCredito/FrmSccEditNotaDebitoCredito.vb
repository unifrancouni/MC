Public Class FrmSccEditNotaDebitoCredito
    ...
    Private Function ValidaDatosEntradaFicha() As Boolean
        ...
        Try
            ...
            strSQL = "SELECT     dbo.SccSolicitudCheque.nSccSolicitudChequeID " & _
" FROM         dbo.SclFichaNotificacionDetalle INNER JOIN " & _
  "                    dbo.SccSolicitudDesembolsoCredito ON " & _
  "                    dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionDetalleID = dbo.SccSolicitudDesembolsoCredito.nSclFichaNotificacionDetalleID INNER JOIN " & _
  "                    dbo.SccSolicitudCheque ON  " & _
  "                    dbo.SccSolicitudDesembolsoCredito.nSccSolicitudDesembolsoCreditoID = dbo.SccSolicitudCheque.nSccSolicitudDesembolsoCreditoID INNER JOIN " & _
  "                    dbo.SccTablaAmortizacion ON " & _
   "                   dbo.SclFichaNotificacionDetalle.nSclFichaNotificacionDetalleID = dbo.SccTablaAmortizacion.nSclFichaNotificacionDetalleID INNER JOIN " & _
    "                  dbo.StbValorCatalogo ON dbo.SccTablaAmortizacion.nStbEstadoPagoID = dbo.StbValorCatalogo.nStbValorCatalogoID " & _
" WHERE     (dbo.SccSolicitudCheque.nSccSolicitudChequeID = " & Me.nSccSolicitudChequeId & ") AND (dbo.StbValorCatalogo.sCodigoInterno = '1')"
            ...
            strSQL = "	SELECT     MAX(dbo.SccReciboOficialCaja.dFechaRecibo) as FechaPagoUltimo " & _
   " FROM         dbo.SccReciboOficialCaja INNER JOIN " & _
 "							  dbo.StbValorCatalogo ON dbo.SccReciboOficialCaja.nStbEstadoReciboID = dbo.StbValorCatalogo.nStbValorCatalogoID " & _
...
" WHERE     (dbo.SccSolicitudCheque.nSccSolicitudChequeID = " & xnSccSolicitudChequeId & ") AND (dbo.StbValorCatalogo.sCodigoInterno = '1')"
            ...
            strSQL = "SELECT     nSccNotaID, nSccSolicitudChequeID, sCodigoInterno FROM         dbo.vwSccNotaCreditoDebitoDetallesFichas  a WHERE    (sCodigoInterno <> '5') And  (nSccSolicitudChequeID = " & xnSccSolicitudChequeId & ") AND (nSccNotaID<>" & Me.IdNota & " )" & _
 ...
 "		WHERE     (dbo.StbValorCatalogo.sCodigoInterno <> '3') AND (dbo.SccReciboOficialCaja.nSccSolicitudChequeID = " & xnSccSolicitudChequeId & " )"
    ...
    Private Function UltimaFuenteIngresada() As Int32
        ...
        sQuery = "SELECT top 1 " & _
        "                sff.nScnFuenteFinanciamientoID," & _
        "                sff.sCodigo," & _
        "                sff.sNombre, " & _
        "                sff.nFondoPresupuestario" & _
        "         FROM   SccNotaCreditoDebito sncd" & _
        "                INNER JOIN ScnFuenteFinanciamiento sff" & _
        "                    ON  sff.nScnFuenteFinanciamientoID = sncd.nScnFuenteFinanciamientoID" & _
        "                INNER JOIN (" & _
        "                      SELECT sc.*" & _
        "                      FROM   SsgCuenta sc" & _
        "                             INNER JOIN SrhEmpleado se" & _
        "                                  ON  se.nSrhEmpleadoID = sc.objEmpleadoID" & _
        "                             INNER JOIN StbPersona sp" & _
        "                                  ON  sp.nStbPersonaID = se.nStbPersonaID" & _
        "                      WHERE  sc.[Login] = '" + InfoSistema.LoginName + "'" & _
        "                ) AS USR ON sncd.nUsuarioCreacionID = USR.SsgCuentaID  " & _
    ...
    Private Sub grdDetalles_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles grdDetalles.Click
    ...
    Private Function BuscarFuenteFinanciamiento() As String
    ...
    Private Sub grdSeleccion_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles grdSeleccion.Click
    ...
    Private Sub cboGrupoS_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboGrupoS.TextChanged
        Try
            ...
            If Me.cboDelegacion.ListCount > 0 Then
                Me.cboDelegacion.SelectedIndex = 0
            End If
            Me.XdtDelegacion.SetCurrentByID("nStbDelegacionProgramaID", Me.XdtGrupoS.ValueField("nStbDelegacionProgramaID"))
            CargarConcepto()
        Catch ex As Exception
            Control_Error(ex)
    ...
    Private Sub cboGrupoS_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboGrupoS.Leave
        ...
        '    SqlStr = "SELECT     'FICHA NO.' + CAST(FNC.nCodigo AS Varchar(10)) +  ' GRUPO: ' + dbo.SclGrupoSolidario.sDescripcion + '(' + dbo.SclGrupoSolidario.sCodigo + ') ACTA DE COMITE No. ' + FNC.sNumSesion + ' CON FECHA DE NOTIFICACION ' + CONVERT(nvarchar(30), FNC.dFechaNotificacion, 105) AS DESCR" & _
        '             " FROM         dbo.SclFichaNotificacionCredito AS FNC INNER JOIN  dbo.SclGrupoSolidario ON FNC.nSclGrupoSolidarioID = dbo.SclGrupoSolidario.nSclGrupoSolidarioID INNER JOIN  dbo.StbValorCatalogo ON FNC.nStbEstadoCreditoID = dbo.StbValorCatalogo.nStbValorCatalogoID " & _
        ...
        '        Me.txtConcepto.Text = IIf(Me.chkEsDebito.Checked, "NOTA DE DEBITO ", "NOTA DE CREDITO ") + RegTmp.ValueField("DESCR")
        '    End If
    ...
    Private Sub CargarConcepto()
        ...
        If cboGrupoS.SelectedIndex > -1 Then
            ...
            xnSclGrupoSolidarioID = Me.XdtGrupoS.ValueField("nSclGrupoSolidarioID")
            SqlStr = "SELECT     'AJUSTE PROPUESTO POR AUDITORIA EXTERNA FICHA NO: ' + CAST(FNC.nCodigo AS Varchar(10)) +  ' GRUPO: ' + dbo.SclGrupoSolidario.sDescripcion + '(' + dbo.SclGrupoSolidario.sCodigo + ')(' + dbo.StbDelegacionPrograma.sNombreDelegacion + ')' AS DESCR" & _
                     " FROM         dbo.SclFichaNotificacionCredito AS FNC INNER JOIN  dbo.SclGrupoSolidario ON FNC.nSclGrupoSolidarioID = dbo.SclGrupoSolidario.nSclGrupoSolidarioID INNER JOIN  dbo.StbValorCatalogo ON FNC.nStbEstadoCreditoID = dbo.StbValorCatalogo.nStbValorCatalogoID  INNER JOIN   dbo.StbDelegacionPrograma ON dbo.SclGrupoSolidario.nStbDelegacionProgramaID = dbo.StbDelegacionPrograma.nStbDelegacionProgramaID" & _
                     " WHERE     (FNC.nSclGrupoSolidarioID = " & xnSclGrupoSolidarioID & ") AND (dbo.StbValorCatalogo.sCodigoInterno = '2')"
    ...
    Private Sub cboGrupoS_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cboGrupoS.KeyPress
