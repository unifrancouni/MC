...
Public Class frmSclEditGS
    ...
    Private Function RegresaCodigoGS() As String
        ...
        Try
            ...
            If ModoForm = "ADD" Then
                strSQL = " SELECT MAX(Codigo) AS maxCodigo " &
                         " FROM  " &
                            " (SELECT RIGHT(sCodigo, 6) AS Codigo  " &
                            " FROM SclGrupoSolidario " &
            ...
            If Not ObjTmpGS("GS").ValueField("maxCodigo") Is DBNull.Value Then
                RegresaCodigoGS = Format(ObjTmpGS("GS").ValueField("maxCodigo") + 1, "000000")
            Else
                RegresaCodigoGS = "001"
            End If
    ...
    Private Sub SalvarGrupoSolidario()
        ...
        Try
            ...
            '------------- FRANCO AQUI ESTABA DIFERENTE NO SE POR QUE
            '-- Encontrar ID de Delegacion con base en el Departamento:
            If ModoForm = "ADD" Then  ' Me.cboDepartamentoV.SelectedIndex <= 0 Then
            ...
            Else
                StrSql = "SELECT StbDelegacionPrograma.nStbDelegacionProgramaID " &
                     "FROM StbDelegacionPrograma INNER JOIN StbMunicipio ON StbDelegacionPrograma.nStbMunicipioID = StbMunicipio.nStbMunicipioID " &
                     "WHERE (StbMunicipio.nStbDepartamentoID = " & Me.cboDepartamentoV.Columns("nStbDepartamentoID").Value & ")"
            End If
            ...
            Else
				...
				GuardarAuditoriaTablas(9, 2, "Modificar Grupo Solidario.", IdSclGrupoSolidario, InfoSistema.IDCuenta)

                StrSql = "UPDATE SclGrupoSolidario " &
                         "SET nSclTipodePlandeNegocioID = " & Me.cboClasificacion.Columns("nSclTipodePlandeNegocioID").Value & ", sCodigo = '" & StrCodigoGS & "', sDescripcion = '" & Trim(RTrim(Me.txtDescripcion.Text)) &
                         "', nStbMercadoVerificadoID = " & cboMercadoV.Columns("nStbMercadoID").Value & ", nStbBarrioVerificadoID = " & cboBarrioV.Columns("nStbBarrioID").Value &
                         ", dFechaModificacion = GetDate(), sUsuarioModificacion = '" & InfoSistema.LoginName &
                         "', nStbDelegacionProgramaID=" & IntDelegacionID &
            ...
            If ModoForm = "ADD" Then '1 Solicitud Cheque agregar 
                GuardarAuditoriaTablas(9, 1, "Agregar Grupo Solidario.", IdSclGrupoSolidario, InfoSistema.IDCuenta)
            End If
    ...
    Private Sub CargarFiltroTipoGrupo(ByVal intMercadoID As Int32, ByRef Combo As C1.Win.C1List.C1Combo)
        Try
            Dim Strsql As String
            Strsql = " SELECT nStbValorCatalogoID," &
                     "        sCodigoInterno," &
                     "        sDescripcion " &
                     " FROM   StbValorCatalogo" &
                     String.Format(" WHERE  (nStbCatalogoID = {0})", TipoGrupo)
        ...
        Try
            ...
            If InfoSistema.IDDelegacion = 1 Then 'Si es delegacion central ver todo'
                If intClasificacionID = 0 Then
                    Strsql = " Select a.nSclTipodePlandeNegocioID, a.nCodigo, a.sDescripcion " &
                         " FROM SclTipodePlandeNegocio a  " &
                         " Where (a.nCreditoIndividual = 0) and (a.nActivo=1)" &
            ...
            Else 'Solo ver hasta ventana de genero no ver PDIBA para las demas delegaciones
                If intClasificacionID = 0 Then
                    Strsql = " Select a.nSclTipodePlandeNegocioID, a.nCodigo, a.sDescripcion " &
                         " FROM SclTipodePlandeNegocio a  " &
                         " Where (a.nCreditoIndividual = 0) and (a.nCodigo<=4) and (a.nActivo=1) " &
                         " Order by a.nSclTipodePlandeNegocioID"
                Else
                    Strsql = " Select a.nSclTipodePlandeNegocioID, a.nCodigo, a.sDescripcion " &
                         " FROM SclTipodePlandeNegocio a  " &
                         " Where ( (a.nCreditoIndividual = 0) and (a.nActivo=1) and  (a.nCodigo<=4) ) or (a.nSclTipodePlandeNegocioID = " & intClasificacionID & ")  " &
                         " Order by a.nSclTipodePlandeNegocioID"
                End If
    ...
    Private Sub CargarBarrio(ByVal intLimpiarID As Integer, ByVal intBarrioID As Integer)
        Try
            ...
            If ckTurismo.Checked Then
                cboBarrio.SelectedIndex = cboBarrio.FindStringExact("No Aplica")
            End If
    ...
    Private Sub ckTurismo_CheckedChanged(sender As Object, e As EventArgs) Handles ckTurismo.CheckedChanged
        If ckTurismo.Checked Then
            If cboBarrio.ListCount = 0 Then
                MsgBox("Primero elija un departamento, un municipio y un distrito.", vbCritical, "Error")
                ckTurismo.Checked = False
            Else
                cboBarrio.SelectedIndex = cboBarrio.FindStringExact("No Aplica")
