...
Public Class frmSclSolicitudDesembolso
    ...
    Dim xdtSegundaFuente As BOSistema.Win.XDataSet.xDataTable
    ...
    Private Sub InicializarVariables()
        Try
            ...
            xdtSegundaFuente = New BOSistema.Win.XDataSet.xDataTable
    ...
    Private Sub CargarSolicitud()
        Try
            ...
            'Segunda Fuente
            Dim idSegundaFuente As Integer
            idSegundaFuente = OptenerDesembolsoSegundaFuente(ObjSolicituddr.IsFieldNull("nSclFichaNotificacionDetalleID"))

            If idSegundaFuente > 0 Then
                Me.cboSegundaFuente.SelectedIndex = idSegundaFuente
            Else
                Me.cboSegundaFuente.SelectedIndex = 0
            End If
    ...
    '----- CARGAR SEGUNDA FUENTE

    Private Sub CargarSegundaFuente(ByVal intFuenteID As Integer)
        Try
            Dim Strsql As String = ""

            '-- Temporal solo permitir seleccionar nueva Fuente de Fondos: BANDES, VENTANA, FONDOS PROPIOS
            If intFuenteID = 0 Then
                Strsql = " Select a.nScnFuenteFinanciamientoSegundaFuenteID, a.sCodigo, a.sNombre, a.nActiva " & _
                         " From ScnFuenteFinanciamientoSegundaFuente a " & _
                         " Where (a.nActiva = 1) " & _
                          " Order by a.sCodigo"
                ''And (a.nScnFuenteFinanciamientoSegundaFuenteID > 5) " & _

            Else
                Strsql = " Select a.nScnFuenteFinanciamientoSegundaFuenteID, a.sCodigo, a.sNombre, a.nActiva " & _
                         " From ScnFuenteFinanciamientoSegundaFuente a " & _
                         " Where (a.nActiva = 1 And  a.nScnFuenteFinanciamientoSegundaFuenteID = " & intFuenteID & ") " & _
                         " Order by a.sCodigo"
            End If

            xdtSegundaFuente.ExecuteSql(Strsql)
            Me.cboSegundaFuente.DataSource = xdtSegundaFuente.Source

            Me.cboSegundaFuente.Splits(0).DisplayColumns("nScnFuenteFinanciamientoSegundaFuenteID").Visible = False
            Me.cboSegundaFuente.Splits(0).DisplayColumns("nActiva").Visible = False
            Me.cboSegundaFuente.Splits(0).DisplayColumns("sCodigo").Style.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center

            'Configurar el combo
            Me.cboSegundaFuente.Splits(0).DisplayColumns("sCodigo").Width = 47
            Me.cboSegundaFuente.Splits(0).DisplayColumns("sNombre").Width = 100

            Me.cboSegundaFuente.Columns("sCodigo").Caption = "Código"
            Me.cboSegundaFuente.Columns("sNombre").Caption = "Descripción"

            Me.cboSegundaFuente.Splits(0).DisplayColumns("sCodigo").HeadingStyle.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
            Me.cboSegundaFuente.Splits(0).DisplayColumns("sNombre").HeadingStyle.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
    ...
    Private Function OptenerDesembolsoSegundaFuente(ByVal FichaNotificacionDetalleID As Integer) As Integer
        Dim XdsDesembolsoSegundaFuente As New BOSistema.Win.XDataSet.xDataTable
        Try
            Dim Strsql As String = "SELECT dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nSclFichaNotificacionDetalleID,dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nSccSolicitudDesembolsoSegundaFuenteCreditoID, dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nScnFuenteFinanciamientoSegundaFuenteID" & _
                                    " FROM dbo.ScnFuenteFinanciamientoSegundaFuente INNER JOIN dbo.SccSolicitudDesembolsoSegundaFuenteCredito" & _
                                    " ON dbo.ScnFuenteFinanciamientoSegundaFuente.nScnFuenteFinanciamientoSegundaFuenteID = dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nScnFuenteFinanciamientoSegundaFuenteID" & _
                                    " WHERE  dbo.SccSolicitudDesembolsoSegundaFuenteCredito.nSclFichaNotificacionDetalleID = " & ObjSolicituddr.nSclFichaNotificacionDetalleID

            XdsDesembolsoSegundaFuente.ExecuteSql(Strsql)
            If XdsDesembolsoSegundaFuente.Count > 0 Then
                Return XdsDesembolsoSegundaFuente.ValueField("nScnFuenteFinanciamientoSegundaFuenteID")
            Else
                Return 0
            End If


            'Return XdsDesembolsoSegundaFuente.
        Catch ex As Exception
            Control_Error(ex)
        End Try
    ...
    Private Function ValidaDatosEntrada() As Boolean
        ...
        Try
            ...
            'Fecha de Solicitud de Desembolso Igual a RESOLUCION
            Strsql2 = "SELECT CONVERT(DATE, dFechaNotificacion, 102) FROM dbo.SclFichaNotificacionCredito WHERE nSclFichaNotificacionID=" & Me.intSclFichaID
            Dim fecha As Date = cmd.ExecuteScalar(Strsql2)
            If Not IsDBNull(fecha) Then
                If Year(Me.cdeFechaSolicitud.Value) <> Year(fecha) Or
                    Month(Me.cdeFechaSolicitud.Value) <> Month(fecha) Or
                    CDate(Me.cdeFechaSolicitud.Value).Day <> fecha.Day Then
                    'Me.intSclFichaID 
                    MsgBox("La Fecha de Desembolso del Crédito DEBE ser igual a la de resolución.", MsgBoxStyle.Critical, NombreSistema)
                    ValidaDatosEntrada = False
                    Me.errSolicitudes.SetError(Me.cdeFechaSolicitud, "La Fecha de Desembolso del Crédito DEBE ser igual a la de resolución.")
                    Me.cdeFechaSolicitud.Focus()
                    Exit Function
                End If
            End If
            ...
            '-- Cambios el 10/12/2015
            ' Fecha de Solicitud y desembolso no puede tener diferencia de más de un mes
            If Month(Format(cdeFechaSolicitud.Value, "yyyy-MM-dd")) - Month(Format(cdeFechaDesembolso.Value, "yyyy-MM-dd")) <> 0 Then
                MsgBox("La diferencia de fechas deben estar en el mismo mes.", MsgBoxStyle.Critical, NombreSistema)
                ValidaDatosEntrada = False
                Exit Function
            End If
            '-- Fin Cambios
            ...
            'Revisar que aun no combinen tipos turismos
            Strsql2 = "select dbo.fnSclPermiteAprobarCreditoTurismo(" & intFichaID & ")"
            Dim ficha = cmd.ExecuteScalar(strSQL2)
            If ficha <> -1 Then
            ...
            If DateDiff(DateInterval.Day, CDate(StrFechaDesembolso), FechaServer) > 15 Then
                MsgBox("La Fecha de desembolso NO DEBE ser mayor  de 15 días respecto a la fecha actual." & Chr(13), MsgBoxStyle.Critical, NombreSistema)
                ValidaDatosEntrada = False
            ...
            'Segunda Fuente
            If Me.cboSegundaFuente.SelectedIndex = -1 Then
                MsgBox("Debe indicar Segunda Fuente de Financiamiento.", MsgBoxStyle.Critical, NombreSistema)
                ValidaDatosEntrada = False
                Me.errSolicitudes.SetError(Me.cboFuente, "Debe indicar Segunda Fuente de Financiamiento.")
                Me.cboSegundaFuente.Focus()
                Exit Function
            End If
    ...
    Private Sub SalvarSolicitudes()
        ...
        Try
            ...
            strSQL = " EXEC spSccGrabarSolicitudDesembolsoSegundaFuente " & Me.intFichaID & ", " & cboFuente.Columns("nScnFuenteFinanciamientoID").Value & "," & cboSegundaFuente.Columns("nScnFuenteFinanciamientoSegundaFuenteID").Value & ", '" & strFechaSolicitud & "', '" & _
