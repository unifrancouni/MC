...
Public Class frmSclParametroGrupoPago
    ...
    Private Sub CargarLugarPago(ByVal intLimpiarID As Integer)
        Try
            ...
            If intLimpiarID = 0 Then
                Strsql = " Select a.nStbPersonaID, a.nCodPersona, a.sNombre, 1 as Orden " &
                         " From vwStbPersona a " &
                         " Where (a.nLugarPagosPrograma = 1) And (a.sCodEmpleado = 'J') " '& _
    ...
    Private Sub CargarDiaPago()
        Try
            ...
            Strsql = " Select a.nStbValorCatalogoID, a.sCodigoInterno, a.sDescripcion,1 as Orden " &
    ...
    Private Function ValidarParametros() As Boolean
        ...
        Try
            ...
            If Me.dFechaInicial.Enabled = True And Me.dFechaFinal.Enabled = True Then

                If Me.dFechaInicial.ValueIsDbNull Then
                    MsgBox("La Fecha inicial  NO DEBE quedar vacía.", MsgBoxStyle.Critical, NombreSistema)
                    VbResultado = False
                    Me.errParametro.SetError(Me.dFechaInicial, "La Fecha inicial NO DEBE quedar vacía.")
                    Me.dFechaInicial.Focus()
                    GoTo salir
                End If


                If Me.dFechaFinal.ValueIsDbNull Then
                    MsgBox("La Fecha final  NO DEBE quedar vacía.", MsgBoxStyle.Critical, NombreSistema)
                    VbResultado = False
                    Me.errParametro.SetError(Me.dFechaFinal, "La Fecha final NO DEBE quedar vacía.")
                    Me.dFechaInicial.Focus()
                    GoTo salir
                End If
        ...
        Try
            ...
            Select Case mNomRep
                ...
                Case EnumReportes.rptSclListGrupoPagoCoordinador
                    ...
                    If RdCarteraVigente.Checked Then
                        If Trim(Filtro) <> "" Then
                            Filtro = Filtro & " And Vigente=1"
                        Else
                            Filtro = " Where  Vigente=1"
                        End If
                    ElseIf RdCarteraVencida.Checked Then
                        If Trim(Filtro) <> "" Then
                            Filtro = Filtro & " And Vigente=0"
                        Else
                            Filtro = " Where  Vigente=0"
                        End If
                    End If
                ...
                Case EnumReportes.rptSclListGrupoPagoDistNumerica
                    ...
                    'Observación:        No se asocia Lugar de Pago con ubicación porque hay grupos de otros distritos que pagan 
                    '                    en otros lugares que "no les corresponde"
                    If cboLugarPago.SelectedIndex > -1 Then
                        If Trim(Filtro) <> "" Then
                            Filtro = Filtro & " And sLugarPago='" & Me.cboLugarPago.Text & "'"
                        Else
                    ...
                    If Me.RdCarteraVencida.Checked Or Me.RdCarteraVigente.Checked Then
                        If Trim(Filtro) <> "" Then
                            Filtro = Filtro & " And nTipoCartera=" & IIf(RdCarteraVigente.Checked, 1, 0)
                        Else
                            Filtro = " Where nTipoCartera=" & IIf(RdCarteraVigente.Checked, 1, 0)
                        End If
                    End If
                    ...
                    'frmVisor.Parametros("@TipoCartera") = IIf(RdCarteraTodos.Checked, -1, IIf(RdCarteraVigente.Checked, 1, 0))
                    ...
                    'frmVisor.Parametros("@DistritoID") = Me.cboDistrito.Columns("nStbDistritoID").Value
                    'frmVisor.Parametros("@nFondoPresupuestarioID") = IIf(RdTodos.Checked, -1, IIf(RdPresupuesto.Checked, 1, 0))
                    'frmVisor.Parametros("@LugarPago") = "'" & Me.cboLugarPago.Text & "'"
