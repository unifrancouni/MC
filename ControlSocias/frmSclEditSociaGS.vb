...
Public Class frmSclEditSociaGS
    ...
    Public Sub CargarSociaGS()
        ...
        Try
            ...
            If Not ObjGrupoSociadr.IsFieldNull("nSclSociaID") Then
                'Cédula:
                strSQL = "SELECT sNumeroCedula FROM SclSocia " &
                         "WHERE (nSclSociaID = " & ObjGrupoSociadr.nSclSociaID & ")"
                Me.mtbNumCedula.Text = XcDatos.ExecuteScalar(strSQL)
                'Nombre:
                strSQL = "SELECT SclSocia.sNombre1 + ' ' + dbo.SclSocia.sNombre2 + ' ' + dbo.SclSocia.sApellido1 + ' ' + dbo.SclSocia.sApellido2 as Nombre " &
    ...
    Private Function ValidaDatosEntrada() As Boolean
        ...
        Try
            ...
            If ModoFrm = "UPD" Then
                Strsql = " SELECT a.nSclSociaID " &
                         " FROM  SclGrupoSocia a INNER JOIN SclGrupoSolidario b ON a.nSclGrupoSolidarioID = b.nSclGrupoSolidarioID INNER JOIN StbValorCatalogo c ON b.nStbEstadoGrupoID = c.nStbValorCatalogoID  " &
            ...
            'Si la socia se encuentra en la CENTRAL DE RIESGO no se acepta integrarse al grupo
            Strsql = "SELECT        dbo.SclReferenciaSocia.nSclSociaID FROM  dbo.SclReferenciaSocia INNER JOIN dbo.SclSocia ON dbo.SclReferenciaSocia.nSclSociaID = dbo.SclSocia.nSclSociaID WHERE        (dbo.SclReferenciaSocia.nActiva = 1) AND dbo.SclReferenciaSocia.nSclSociaID = " & txtNombreSocia.Tag
            XdtSclFicha.ExecuteSql(Strsql)
            If XdtSclFicha.Count > 0 Then
                MsgBox("No se puede agregar socia porque se encuentra ACTIVA en la CENTRAL DE RIESGO.", vbCritical, "Error")
                ValidaDatosEntrada = False
                Exit Function
            End If

            'Si la socia se encuentra SUPERVISADA no se acepta integrarse al grupo
            Strsql = "SELECT * FROM vwSclSupervision WHERE nActivo=1 and nSclSociaID=" & txtNombreSocia.Tag
            XdtSclFicha.ExecuteSql(Strsql)
            If XdtSclFicha.Count > 0 Then
                MsgBox("No se puede agregar socia porque se encuentra SUPERVISADA.", vbCritical, "Error")
                ValidaDatosEntrada = False
                Exit Function
            End If
            ...
            Strsql = " SELECT c.nSclGrupoSolidarioID, SclSocia.sNombre1 + ' ' + SclSocia.sNombre2 + ' ' + SclSocia.sApellido1 + ' ' + SclSocia.sApellido2 AS Socia,  SclSocia.sNumeroCedula " &
                     " FROM SclFichaSocia AS a INNER JOIN StbValorCatalogo AS b ON a.nStbEstadoFichaID = b.nStbValorCatalogoID INNER JOIN SclGrupoSocia AS c ON a.nSclGrupoSociaID = c.nSclGrupoSociaID INNER JOIN SclSocia ON c.nSclSociaID = SclSocia.nSclSociaID " &
                     " WHERE (b.sCodigoInterno <> '4') AND (b.sCodigoInterno <> '6') AND (b.sCodigoInterno <> '7') " &
                     " AND (c.nSclGrupoSolidarioID IN  (SELECT SclGrupoSocia.nSclGrupoSolidarioID FROM  SclGrupoSocia INNER JOIN SclFichaSocia ON SclGrupoSocia.nSclGrupoSociaID = SclFichaSocia.nSclGrupoSociaID INNER JOIN StbValorCatalogo ON SclFichaSocia.nStbEstadoFichaID = StbValorCatalogo.nStbValorCatalogoID " &
    ...
    Public Sub SalvarSocia()
        Try
            ...
            If ModoForm <> "ADD" Then
                GuardarAuditoriaTablas(8, 2, "Modificar Socia en Grupo Solidario.", IdSclSocia, InfoSistema.IDCuenta)
            End If

            ObjGrupoSociadr.Update()

            'Asignar el Id del Grupo Socia a la variable publica del formulario:
            ...
            If ModoForm = "ADD" Then '1 Solicitud Cheque agregar 
                GuardarAuditoriaTablas(8, 1, "Agregar Socia a Grupo Solidario.", IdSclSocia, InfoSistema.IDCuenta)
            End If
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        Try
            ...
            XContadorReg = 0
            Strsql = " SELECT a.nSclSociaID, a.sNumeroCedula, a.NombreSocia " &
                     " FROM vwSclSocia a " &
                     " WHERE (a.nSociaActiva =1) and (a.nDispuestaFormarGS = 1) " &
                     " AND (a.sNumeroCedula = '" & Me.mtbNumCedula.Text & "') " &
